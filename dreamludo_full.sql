-- ================================================================
--  Dream Ludo — Complete Database Schema
--  Upload and import on your server:
--    mysql -u root -p dreamludo < dreamludo_full.sql
-- ================================================================
...
CREATE TABLE IF NOT EXISTS `tbl_app_configuration` (
  ...
  `package_name` varchar(100) DEFAULT 'com.definiteautomation.dreamludofantacy',
  `how_to_play` text,
...
INSERT INTO `tbl_app_configuration`
  (`id`,`app_name`,`currency_code`,`currency_sign`,`country_code`,`time_zone`,
   `game_name`,`how_to_play`,`latest_version_name`,`latest_version_code`)
VALUES
  (1,'Dream Ludo','INR','₹','91','Asia/Kolkata','Ludo',
   'Challenge your friends in Ludo matches and win real money!','1.0',1)
ON DUPLICATE KEY UPDATE `id`=`id`;


-- ----------------------------------------------------------------
-- Content Pages (FAQ, Terms, Privacy, About, Rules, Legal)
-- ----------------------------------------------------------------
...
(1,'<p>FAQ content here.</p>','<p>Terms &amp; Conditions here.</p>',
   '<p>Privacy Policy here.</p>','<p>About us here.</p>',
   '<p>Game rules here.</p>','<p>Legal policy here.</p>')
ON DUPLICATE KEY UPDATE `id`=`id`;


-- ----------------------------------------------------------------
-- Users  (admin + player accounts)
-- ----------------------------------------------------------------
...
-- Generate your own SHA1: echo -n "YourPassword" | sha1sum
...
CREATE TABLE IF NOT EXISTS `tbl_match` (
  ...
  `date_created` datetime DEFAULT CURRENT_TIMESTAMP,
  `coin_id` int(11) DEFAULT NULL COMMENT 'NULL = fiat match',
  `coin_fee` decimal(24,9) DEFAULT NULL COMMENT 'Entry fee in coins',
  `coin_prize` decimal(24,9) DEFAULT NULL COMMENT 'Prize in coins',
...
CREATE TABLE IF NOT EXISTS `tbl_participants` (
  ...
  `result_status` tinyint(1) DEFAULT NULL COMMENT '1=declared',
...
-- ----------------------------------------------------------------
-- Coin Types  (Solana, SPL tokens, custom coins)
-- ----------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `tbl_coin_type` (
  ...
  `contract_address` varchar(255) DEFAULT NULL COMMENT 'SPL token mint address',
  `rpc_url` varchar(500) DEFAULT NULL,
  `rpc_backup` varchar(500) DEFAULT NULL,
  `decimals` int(11) DEFAULT 9,
  `icon_url` varchar(500) DEFAULT NULL,
  `admin_wallet` varchar(255) DEFAULT NULL COMMENT 'Platform hot-wallet address',
  `admin_private_key` text DEFAULT NULL COMMENT 'Encrypted private key',
...
INSERT INTO `tbl_coin_type`
  (`id`,`name`,`symbol`,`type`,`rpc_url`,`decimals`,`network`,`is_active`,`is_default`,`sort_order`)
VALUES
  (1,'Solana','SOL','solana','https://api.mainnet-beta.solana.com',9,'mainnet',1,1,1)
ON DUPLICATE KEY UPDATE `id`=`id`;


-- ----------------------------------------------------------------
-- User Coin Wallets  (balance per user per coin)
-- ----------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `tbl_coin_wallet` (
  ...
  `wallet_address` varchar(255) DEFAULT NULL COMMENT 'User on-chain address',
  `date_created` datetime DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS `tbl_coin_transaction` (
  ...
  `ref_id` varchar(255) DEFAULT NULL COMMENT 'match_id or tx_hash',
  `tx_hash` varchar(255) DEFAULT NULL COMMENT 'on-chain signature',
...
-- ----------------------------------------------------------------
-- Betting Bank Wallet  (escrow pot held per match)
-- ----------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `tbl_coin_betting_bank` (
  ...
  `status` enum('held','won','refunded','cancelled') DEFAULT 'held'
    COMMENT 'held=in match, won=paid to winner, refunded=match cancelled',
  `date_created` datetime DEFAULT CURRENT_TIMESTAMP,
...
-- ================================================================
--  END OF SCHEMA
--  After import, update your .env with real credentials and
--  change the admin password via the admin panel.
-- ================================================================
