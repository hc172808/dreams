.class public final Lokhttp3/internal/ws/RealWebSocket;
.super Ljava/lang/Object;
.source "RealWebSocket.kt"

# interfaces
.implements Lokhttp3/WebSocket;
.implements Lokhttp3/internal/ws/WebSocketReader$FrameCallback;


# annotations
.annotation system Ldalvik/annotation/MemberClasses;
    value = {
        Lokhttp3/internal/ws/RealWebSocket$Message;,
        Lokhttp3/internal/ws/RealWebSocket$Close;,
        Lokhttp3/internal/ws/RealWebSocket$Streams;,
        Lokhttp3/internal/ws/RealWebSocket$WriterTask;,
        Lokhttp3/internal/ws/RealWebSocket$Companion;
    }
.end annotation

.annotation system Ldalvik/annotation/SourceDebugExtension;
    value = "SMAP\nRealWebSocket.kt\nKotlin\n*S Kotlin\n*F\n+ 1 RealWebSocket.kt\nokhttp3/internal/ws/RealWebSocket\n+ 2 TaskQueue.kt\nokhttp3/internal/concurrent/TaskQueue\n+ 3 Util.kt\nokhttp3/internal/Util\n*L\n1#1,571:1\n84#2,4:572\n93#2,10:580\n573#3,4:576\n*E\n*S KotlinDebug\n*F\n+ 1 RealWebSocket.kt\nokhttp3/internal/ws/RealWebSocket\n*L\n216#1,4:572\n427#1,10:580\n397#1,4:576\n*E\n"
.end annotation

.annotation runtime Lkotlin/Metadata;
    bv = {
        0x1,
        0x0,
        0x3
    }
    d1 = {
        "\u0000\u00b0\u0001\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010\t\n\u0002\u0008\u0002\n\u0002\u0010\u000b\n\u0000\n\u0002\u0018\u0002\n\u0002\u0008\u0003\n\u0002\u0010\u000e\n\u0002\u0008\u0003\n\u0002\u0018\u0002\n\u0002\u0010\u0000\n\u0002\u0008\u0002\n\u0002\u0018\u0002\n\u0002\u0008\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010\u0008\n\u0002\u0008\u0005\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010\u0002\n\u0002\u0008\u0002\n\u0002\u0018\u0002\n\u0002\u0008\u0003\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\u0008\u0007\n\u0002\u0018\u0002\n\u0002\u0008\u0002\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0008\u001b\u0018\u0000 \\2\u00020\u00012\u00020\u0002:\u0005[\\]^_B-\u0012\u0006\u0010\u0003\u001a\u00020\u0004\u0012\u0006\u0010\u0005\u001a\u00020\u0006\u0012\u0006\u0010\u0007\u001a\u00020\u0008\u0012\u0006\u0010\t\u001a\u00020\n\u0012\u0006\u0010\u000b\u001a\u00020\u000c\u00a2\u0006\u0002\u0010\rJ\u0016\u0010/\u001a\u0002002\u0006\u00101\u001a\u00020\u000c2\u0006\u00102\u001a\u000203J\u0008\u00104\u001a\u000200H\u0016J\u001f\u00105\u001a\u0002002\u0006\u00106\u001a\u0002072\u0008\u00108\u001a\u0004\u0018\u000109H\u0000\u00a2\u0006\u0002\u0008:J\u001a\u0010;\u001a\u00020\u000f2\u0006\u0010<\u001a\u00020\"2\u0008\u0010=\u001a\u0004\u0018\u00010\u0015H\u0016J \u0010;\u001a\u00020\u000f2\u0006\u0010<\u001a\u00020\"2\u0008\u0010=\u001a\u0004\u0018\u00010\u00152\u0006\u0010>\u001a\u00020\u000cJ\u000e\u0010?\u001a\u0002002\u0006\u0010@\u001a\u00020AJ\u001c\u0010B\u001a\u0002002\n\u0010C\u001a\u00060Dj\u0002`E2\u0008\u00106\u001a\u0004\u0018\u000107J\u0016\u0010F\u001a\u0002002\u0006\u0010\u001b\u001a\u00020\u00152\u0006\u0010\'\u001a\u00020(J\u0006\u0010G\u001a\u000200J\u0018\u0010H\u001a\u0002002\u0006\u0010<\u001a\u00020\"2\u0006\u0010=\u001a\u00020\u0015H\u0016J\u0010\u0010I\u001a\u0002002\u0006\u0010J\u001a\u00020\u0015H\u0016J\u0010\u0010I\u001a\u0002002\u0006\u0010K\u001a\u00020\u001dH\u0016J\u0010\u0010L\u001a\u0002002\u0006\u0010M\u001a\u00020\u001dH\u0016J\u0010\u0010N\u001a\u0002002\u0006\u0010M\u001a\u00020\u001dH\u0016J\u000e\u0010O\u001a\u00020\u000f2\u0006\u0010M\u001a\u00020\u001dJ\u0006\u0010P\u001a\u00020\u000fJ\u0008\u0010\u001e\u001a\u00020\u000cH\u0016J\u0006\u0010$\u001a\u00020\"J\u0006\u0010%\u001a\u00020\"J\u0008\u0010Q\u001a\u00020\u0006H\u0016J\u0008\u0010R\u001a\u000200H\u0002J\u0010\u0010S\u001a\u00020\u000f2\u0006\u0010J\u001a\u00020\u0015H\u0016J\u0010\u0010S\u001a\u00020\u000f2\u0006\u0010K\u001a\u00020\u001dH\u0016J\u0018\u0010S\u001a\u00020\u000f2\u0006\u0010T\u001a\u00020\u001d2\u0006\u0010U\u001a\u00020\"H\u0002J\u0006\u0010&\u001a\u00020\"J\u0006\u0010V\u001a\u000200J\r\u0010W\u001a\u00020\u000fH\u0000\u00a2\u0006\u0002\u0008XJ\r\u0010Y\u001a\u000200H\u0000\u00a2\u0006\u0002\u0008ZR\u000e\u0010\u000e\u001a\u00020\u000fX\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u0010\u0010\u0010\u001a\u0004\u0018\u00010\u0011X\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u000e\u0010\u0012\u001a\u00020\u000fX\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u000e\u0010\u0013\u001a\u00020\u000fX\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u000e\u0010\u0014\u001a\u00020\u0015X\u0082\u0004\u00a2\u0006\u0002\n\u0000R\u0014\u0010\u0007\u001a\u00020\u0008X\u0080\u0004\u00a2\u0006\u0008\n\u0000\u001a\u0004\u0008\u0016\u0010\u0017R\u0014\u0010\u0018\u001a\u0008\u0012\u0004\u0012\u00020\u001a0\u0019X\u0082\u0004\u00a2\u0006\u0002\n\u0000R\u0010\u0010\u001b\u001a\u0004\u0018\u00010\u0015X\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u000e\u0010\u0005\u001a\u00020\u0006X\u0082\u0004\u00a2\u0006\u0002\n\u0000R\u000e\u0010\u000b\u001a\u00020\u000cX\u0082\u0004\u00a2\u0006\u0002\n\u0000R\u0014\u0010\u001c\u001a\u0008\u0012\u0004\u0012\u00020\u001d0\u0019X\u0082\u0004\u00a2\u0006\u0002\n\u0000R\u000e\u0010\u001e\u001a\u00020\u000cX\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u000e\u0010\t\u001a\u00020\nX\u0082\u0004\u00a2\u0006\u0002\n\u0000R\u0010\u0010\u001f\u001a\u0004\u0018\u00010 X\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u000e\u0010!\u001a\u00020\"X\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u0010\u0010#\u001a\u0004\u0018\u00010\u0015X\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u000e\u0010$\u001a\u00020\"X\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u000e\u0010%\u001a\u00020\"X\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u000e\u0010&\u001a\u00020\"X\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u0010\u0010\'\u001a\u0004\u0018\u00010(X\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u000e\u0010)\u001a\u00020*X\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u0010\u0010+\u001a\u0004\u0018\u00010,X\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u0010\u0010-\u001a\u0004\u0018\u00010.X\u0082\u000e\u00a2\u0006\u0002\n\u0000\u00a8\u0006`"
    }
    d2 = {
        "Lokhttp3/internal/ws/RealWebSocket;",
        "Lokhttp3/WebSocket;",
        "Lokhttp3/internal/ws/WebSocketReader$FrameCallback;",
        "taskRunner",
        "Lokhttp3/internal/concurrent/TaskRunner;",
        "originalRequest",
        "Lokhttp3/Request;",
        "listener",
        "Lokhttp3/WebSocketListener;",
        "random",
        "Ljava/util/Random;",
        "pingIntervalMillis",
        "",
        "(Lokhttp3/internal/concurrent/TaskRunner;Lokhttp3/Request;Lokhttp3/WebSocketListener;Ljava/util/Random;J)V",
        "awaitingPong",
        "",
        "call",
        "Lokhttp3/Call;",
        "enqueuedClose",
        "failed",
        "key",
        "",
        "getListener$okhttp",
        "()Lokhttp3/WebSocketListener;",
        "messageAndCloseQueue",
        "Ljava/util/ArrayDeque;",
        "",
        "name",
        "pongQueue",
        "Lokio/ByteString;",
        "queueSize",
        "reader",
        "Lokhttp3/internal/ws/WebSocketReader;",
        "receivedCloseCode",
        "",
        "receivedCloseReason",
        "receivedPingCount",
        "receivedPongCount",
        "sentPingCount",
        "streams",
        "Lokhttp3/internal/ws/RealWebSocket$Streams;",
        "taskQueue",
        "Lokhttp3/internal/concurrent/TaskQueue;",
        "writer",
        "Lokhttp3/internal/ws/WebSocketWriter;",
        "writerTask",
        "Lokhttp3/internal/concurrent/Task;",
        "awaitTermination",
        "",
        "timeout",
        "timeUnit",
        "Ljava/util/concurrent/TimeUnit;",
        "cancel",
        "checkUpgradeSuccess",
        "response",
        "Lokhttp3/Response;",
        "exchange",
        "Lokhttp3/internal/connection/Exchange;",
        "checkUpgradeSuccess$okhttp",
        "close",
        "code",
        "reason",
        "cancelAfterCloseMillis",
        "connect",
        "client",
        "Lokhttp3/OkHttpClient;",
        "failWebSocket",
        "e",
        "Ljava/lang/Exception;",
        "Lkotlin/Exception;",
        "initReaderAndWriter",
        "loopReader",
        "onReadClose",
        "onReadMessage",
        "text",
        "bytes",
        "onReadPing",
        "payload",
        "onReadPong",
        "pong",
        "processNextFrame",
        "request",
        "runWriter",
        "send",
        "data",
        "formatOpcode",
        "tearDown",
        "writeOneFrame",
        "writeOneFrame$okhttp",
        "writePingFrame",
        "writePingFrame$okhttp",
        "Close",
        "Companion",
        "Message",
        "Streams",
        "WriterTask",
        "okhttp"
    }
    k = 0x1
    mv = {
        0x1,
        0x1,
        0x10
    }
.end annotation


# static fields
.field private static final CANCEL_AFTER_CLOSE_MILLIS:J = 0xea60L

.field public static final Companion:Lokhttp3/internal/ws/RealWebSocket$Companion;

.field private static final MAX_QUEUE_SIZE:J = 0x1000000L

.field private static final ONLY_HTTP1:Ljava/util/List;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Ljava/util/List<",
            "Lokhttp3/Protocol;",
            ">;"
        }
    .end annotation
.end field


# instance fields
.field private awaitingPong:Z

.field private call:Lokhttp3/Call;

.field private enqueuedClose:Z

.field private failed:Z

.field private final key:Ljava/lang/String;

.field private final listener:Lokhttp3/WebSocketListener;

.field private final messageAndCloseQueue:Ljava/util/ArrayDeque;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Ljava/util/ArrayDeque<",
            "Ljava/lang/Object;",
            ">;"
        }
    .end annotation
.end field

.field private name:Ljava/lang/String;

.field private final originalRequest:Lokhttp3/Request;

.field private final pingIntervalMillis:J

.field private final pongQueue:Ljava/util/ArrayDeque;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Ljava/util/ArrayDeque<",
            "Lokio/ByteString;",
            ">;"
        }
    .end annotation
.end field

.field private queueSize:J

.field private final random:Ljava/util/Random;

.field private reader:Lokhttp3/internal/ws/WebSocketReader;

.field private receivedCloseCode:I

.field private receivedCloseReason:Ljava/lang/String;

.field private receivedPingCount:I

.field private receivedPongCount:I

.field private sentPingCount:I

.field private streams:Lokhttp3/internal/ws/RealWebSocket$Streams;

.field private taskQueue:Lokhttp3/internal/concurrent/TaskQueue;

.field private writer:Lokhttp3/internal/ws/WebSocketWriter;

.field private writerTask:Lokhttp3/internal/concurrent/Task;


# direct methods
.method static constructor <clinit>()V
    .locals 2

    new-instance v0, Lokhttp3/internal/ws/RealWebSocket$Companion;

    const/4 v1, 0x0

    invoke-direct {v0, v1}, Lokhttp3/internal/ws/RealWebSocket$Companion;-><init>(Lkotlin/jvm/internal/DefaultConstructorMarker;)V

    sput-object v0, Lokhttp3/internal/ws/RealWebSocket;->Companion:Lokhttp3/internal/ws/RealWebSocket$Companion;

    .line 556
    sget-object v0, Lokhttp3/Protocol;->HTTP_1_1:Lokhttp3/Protocol;

    invoke-static {v0}, Lkotlin/collections/CollectionsKt;->listOf(Ljava/lang/Object;)Ljava/util/List;

    move-result-object v0

    sput-object v0, Lokhttp3/internal/ws/RealWebSocket;->ONLY_HTTP1:Ljava/util/List;

    return-void
.end method

.method public constructor <init>(Lokhttp3/internal/concurrent/TaskRunner;Lokhttp3/Request;Lokhttp3/WebSocketListener;Ljava/util/Random;J)V
    .locals 7
    .param p1, "taskRunner"    # Lokhttp3/internal/concurrent/TaskRunner;
    .param p2, "originalRequest"    # Lokhttp3/Request;
    .param p3, "listener"    # Lokhttp3/WebSocketListener;
    .param p4, "random"    # Ljava/util/Random;
    .param p5, "pingIntervalMillis"    # J

    const-string v0, "taskRunner"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkParameterIsNotNull(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v0, "originalRequest"

    invoke-static {p2, v0}, Lkotlin/jvm/internal/Intrinsics;->checkParameterIsNotNull(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v0, "listener"

    invoke-static {p3, v0}, Lkotlin/jvm/internal/Intrinsics;->checkParameterIsNotNull(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v0, "random"

    invoke-static {p4, v0}, Lkotlin/jvm/internal/Intrinsics;->checkParameterIsNotNull(Ljava/lang/Object;Ljava/lang/String;)V

    .line 54
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    iput-object p2, p0, Lokhttp3/internal/ws/RealWebSocket;->originalRequest:Lokhttp3/Request;

    iput-object p3, p0, Lokhttp3/internal/ws/RealWebSocket;->listener:Lokhttp3/WebSocketListener;

    iput-object p4, p0, Lokhttp3/internal/ws/RealWebSocket;->random:Ljava/util/Random;

    iput-wide p5, p0, Lokhttp3/internal/ws/RealWebSocket;->pingIntervalMillis:J

    .line 79
    invoke-virtual {p1}, Lokhttp3/internal/concurrent/TaskRunner;->newQueue()Lokhttp3/internal/concurrent/TaskQueue;

    move-result-object v0

    iput-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->taskQueue:Lokhttp3/internal/concurrent/TaskQueue;

    .line 92
    new-instance v0, Ljava/util/ArrayDeque;

    invoke-direct {v0}, Ljava/util/ArrayDeque;-><init>()V

    iput-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->pongQueue:Ljava/util/ArrayDeque;

    .line 95
    new-instance v0, Ljava/util/ArrayDeque;

    invoke-direct {v0}, Ljava/util/ArrayDeque;-><init>()V

    iput-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->messageAndCloseQueue:Ljava/util/ArrayDeque;

    .line 104
    const/4 v0, -0x1

    iput v0, p0, Lokhttp3/internal/ws/RealWebSocket;->receivedCloseCode:I

    .line 125
    invoke-virtual {p2}, Lokhttp3/Request;->method()Ljava/lang/String;

    move-result-object v0

    const-string v1, "GET"

    invoke-static {v1, v0}, Lkotlin/jvm/internal/Intrinsics;->areEqual(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v0

    if-eqz v0, :cond_0

    .line 129
    sget-object v1, Lokio/ByteString;->Companion:Lokio/ByteString$Companion;

    const/16 v0, 0x10

    new-array v2, v0, [B

    move-object v0, v2

    .local v0, "$this$apply":[B
    const/4 v3, 0x0

    .local v3, "$i$a$-apply-RealWebSocket$2":I
    invoke-virtual {p4, v0}, Ljava/util/Random;->nextBytes([B)V

    .end local v0    # "$this$apply":[B
    .end local v3    # "$i$a$-apply-RealWebSocket$2":I
    const/4 v3, 0x0

    const/4 v4, 0x0

    const/4 v5, 0x3

    const/4 v6, 0x0

    invoke-static/range {v1 .. v6}, Lokio/ByteString$Companion;->of$default(Lokio/ByteString$Companion;[BIIILjava/lang/Object;)Lokio/ByteString;

    move-result-object v0

    invoke-virtual {v0}, Lokio/ByteString;->base64()Ljava/lang/String;

    move-result-object v0

    iput-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->key:Ljava/lang/String;

    .line 130
    return-void

    .line 125
    :cond_0
    const/4 v0, 0x0

    .line 126
    .local v0, "$i$a$-require-RealWebSocket$1":I
    new-instance v1, Ljava/lang/StringBuilder;

    invoke-direct {v1}, Ljava/lang/StringBuilder;-><init>()V

    const-string v2, "Request must be GET: "

    invoke-virtual {v1, v2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {p2}, Lokhttp3/Request;->method()Ljava/lang/String;

    move-result-object v2

    invoke-virtual {v1, v2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v1}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v0

    .line 125
    .end local v0    # "$i$a$-require-RealWebSocket$1":I
    new-instance v1, Ljava/lang/IllegalArgumentException;

    invoke-virtual {v0}, Ljava/lang/Object;->toString()Ljava/lang/String;

    move-result-object v0

    invoke-direct {v1, v0}, Ljava/lang/IllegalArgumentException;-><init>(Ljava/lang/String;)V

    check-cast v1, Ljava/lang/Throwable;

    throw v1
.end method

.method public static final synthetic access$getName$p(Lokhttp3/internal/ws/RealWebSocket;)Ljava/lang/String;
    .locals 1
    .param p0, "$this"    # Lokhttp3/internal/ws/RealWebSocket;

    .line 54
    iget-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->name:Ljava/lang/String;

    return-object v0
.end method

.method public static final synthetic access$setName$p(Lokhttp3/internal/ws/RealWebSocket;Ljava/lang/String;)V
    .locals 0
    .param p0, "$this"    # Lokhttp3/internal/ws/RealWebSocket;
    .param p1, "<set-?>"    # Ljava/lang/String;

    .line 54
    iput-object p1, p0, Lokhttp3/internal/ws/RealWebSocket;->name:Ljava/lang/String;

    return-void
.end method

.method private final runWriter()V
    .locals 9

    .line 397
    move-object v0, p0

    .local v0, "$this$assertThreadHoldsLock$iv":Ljava/lang/Object;
    const/4 v1, 0x0

    .line 576
    .local v1, "$i$f$assertThreadHoldsLock":I
    sget-boolean v2, Lokhttp3/internal/Util;->assertionsEnabled:Z

    if-eqz v2, :cond_1

    invoke-static {v0}, Ljava/lang/Thread;->holdsLock(Ljava/lang/Object;)Z

    move-result v2

    if-eqz v2, :cond_0

    goto :goto_0

    .line 577
    :cond_0
    new-instance v2, Ljava/lang/AssertionError;

    new-instance v3, Ljava/lang/StringBuilder;

    invoke-direct {v3}, Ljava/lang/StringBuilder;-><init>()V

    const-string v4, "Thread "

    invoke-virtual {v3, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-static {}, Ljava/lang/Thread;->currentThread()Ljava/lang/Thread;

    move-result-object v4

    const-string v5, "Thread.currentThread()"

    invoke-static {v4, v5}, Lkotlin/jvm/internal/Intrinsics;->checkExpressionValueIsNotNull(Ljava/lang/Object;Ljava/lang/String;)V

    invoke-virtual {v4}, Ljava/lang/Thread;->getName()Ljava/lang/String;

    move-result-object v4

    invoke-virtual {v3, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v4, " MUST hold lock on "

    invoke-virtual {v3, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v3, v0}, Ljava/lang/StringBuilder;->append(Ljava/lang/Object;)Ljava/lang/StringBuilder;

    invoke-virtual {v3}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v3

    invoke-direct {v2, v3}, Ljava/lang/AssertionError;-><init>(Ljava/lang/Object;)V

    check-cast v2, Ljava/lang/Throwable;

    throw v2

    .line 579
    :cond_1
    :goto_0
    nop

    .line 399
    .end local v0    # "$this$assertThreadHoldsLock$iv":Ljava/lang/Object;
    .end local v1    # "$i$f$assertThreadHoldsLock":I
    iget-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->writerTask:Lokhttp3/internal/concurrent/Task;

    .line 400
    .local v0, "writerTask":Lokhttp3/internal/concurrent/Task;
    if-eqz v0, :cond_2

    .line 401
    iget-object v3, p0, Lokhttp3/internal/ws/RealWebSocket;->taskQueue:Lokhttp3/internal/concurrent/TaskQueue;

    const-wide/16 v5, 0x0

    const/4 v7, 0x2

    const/4 v8, 0x0

    move-object v4, v0

    invoke-static/range {v3 .. v8}, Lokhttp3/internal/concurrent/TaskQueue;->schedule$default(Lokhttp3/internal/concurrent/TaskQueue;Lokhttp3/internal/concurrent/Task;JILjava/lang/Object;)V

    .line 403
    :cond_2
    return-void
.end method

.method private final declared-synchronized send(Lokio/ByteString;I)Z
    .locals 6
    .param p1, "data"    # Lokio/ByteString;
    .param p2, "formatOpcode"    # I

    monitor-enter p0

    .line 342
    :try_start_0
    iget-boolean v0, p0, Lokhttp3/internal/ws/RealWebSocket;->failed:Z

    const/4 v1, 0x0

    if-nez v0, :cond_2

    iget-boolean v0, p0, Lokhttp3/internal/ws/RealWebSocket;->enqueuedClose:Z

    if-eqz v0, :cond_0

    goto :goto_0

    .line 345
    :cond_0
    iget-wide v2, p0, Lokhttp3/internal/ws/RealWebSocket;->queueSize:J

    invoke-virtual {p1}, Lokio/ByteString;->size()I

    move-result v0

    int-to-long v4, v0

    add-long/2addr v2, v4

    const-wide/32 v4, 0x1000000

    cmp-long v0, v2, v4

    if-lez v0, :cond_1

    .line 346
    const/16 v0, 0x3e9

    const/4 v2, 0x0

    invoke-virtual {p0, v0, v2}, Lokhttp3/internal/ws/RealWebSocket;->close(ILjava/lang/String;)Z
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    .line 347
    monitor-exit p0

    return v1

    .line 351
    .end local p0    # "this":Lokhttp3/internal/ws/RealWebSocket;
    :cond_1
    :try_start_1
    iget-wide v0, p0, Lokhttp3/internal/ws/RealWebSocket;->queueSize:J

    invoke-virtual {p1}, Lokio/ByteString;->size()I

    move-result v2

    int-to-long v2, v2

    add-long/2addr v0, v2

    iput-wide v0, p0, Lokhttp3/internal/ws/RealWebSocket;->queueSize:J

    .line 352
    iget-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->messageAndCloseQueue:Ljava/util/ArrayDeque;

    new-instance v1, Lokhttp3/internal/ws/RealWebSocket$Message;

    invoke-direct {v1, p2, p1}, Lokhttp3/internal/ws/RealWebSocket$Message;-><init>(ILokio/ByteString;)V

    invoke-virtual {v0, v1}, Ljava/util/ArrayDeque;->add(Ljava/lang/Object;)Z

    .line 353
    invoke-direct {p0}, Lokhttp3/internal/ws/RealWebSocket;->runWriter()V
    :try_end_1
    .catchall {:try_start_1 .. :try_end_1} :catchall_0

    .line 354
    const/4 v0, 0x1

    monitor-exit p0

    return v0

    .line 342
    :cond_2
    :goto_0
    monitor-exit p0

    return v1

    .line 341
    .end local p1    # "data":Lokio/ByteString;
    .end local p2    # "formatOpcode":I
    :catchall_0
    move-exception p1

    monitor-exit p0

    throw p1
.end method


# virtual methods
.method public final awaitTermination(JLjava/util/concurrent/TimeUnit;)V
    .locals 1
    .param p1, "timeout"    # J
    .param p3, "timeUnit"    # Ljava/util/concurrent/TimeUnit;
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Ljava/lang/InterruptedException;
        }
    .end annotation

    const-string v0, "timeUnit"

    invoke-static {p3, v0}, Lkotlin/jvm/internal/Intrinsics;->checkParameterIsNotNull(Ljava/lang/Object;Ljava/lang/String;)V

    .line 263
    iget-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->taskQueue:Lokhttp3/internal/concurrent/TaskQueue;

    invoke-virtual {v0}, Lokhttp3/internal/concurrent/TaskQueue;->idleLatch()Ljava/util/concurrent/CountDownLatch;

    move-result-object v0

    invoke-virtual {v0, p1, p2, p3}, Ljava/util/concurrent/CountDownLatch;->await(JLjava/util/concurrent/TimeUnit;)Z

    .line 264
    return-void
.end method

.method public cancel()V
    .locals 1

    .line 137
    iget-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->call:Lokhttp3/Call;

    if-nez v0, :cond_0

    invoke-static {}, Lkotlin/jvm/internal/Intrinsics;->throwNpe()V

    :cond_0
    invoke-interface {v0}, Lokhttp3/Call;->cancel()V

    .line 138
    return-void
.end method

.method public final checkUpgradeSuccess$okhttp(Lokhttp3/Response;Lokhttp3/internal/connection/Exchange;)V
    .locals 8
    .param p1, "response"    # Lokhttp3/Response;
    .param p2, "exchange"    # Lokhttp3/internal/connection/Exchange;
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Ljava/io/IOException;
        }
    .end annotation

    const-string v0, "response"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkParameterIsNotNull(Ljava/lang/Object;Ljava/lang/String;)V

    .line 185
    invoke-virtual {p1}, Lokhttp3/Response;->code()I

    move-result v0

    const/16 v1, 0x27

    const/16 v2, 0x65

    if-ne v0, v2, :cond_4

    .line 190
    const-string v0, "Connection"

    const/4 v2, 0x0

    const/4 v3, 0x2

    invoke-static {p1, v0, v2, v3, v2}, Lokhttp3/Response;->header$default(Lokhttp3/Response;Ljava/lang/String;Ljava/lang/String;ILjava/lang/Object;)Ljava/lang/String;

    move-result-object v0

    .line 191
    .local v0, "headerConnection":Ljava/lang/String;
    const-string v4, "Upgrade"

    const/4 v5, 0x1

    invoke-static {v4, v0, v5}, Lkotlin/text/StringsKt;->equals(Ljava/lang/String;Ljava/lang/String;Z)Z

    move-result v6

    if-eqz v6, :cond_3

    .line 196
    invoke-static {p1, v4, v2, v3, v2}, Lokhttp3/Response;->header$default(Lokhttp3/Response;Ljava/lang/String;Ljava/lang/String;ILjava/lang/Object;)Ljava/lang/String;

    move-result-object v4

    .line 197
    .local v4, "headerUpgrade":Ljava/lang/String;
    const-string v6, "websocket"

    invoke-static {v6, v4, v5}, Lkotlin/text/StringsKt;->equals(Ljava/lang/String;Ljava/lang/String;Z)Z

    move-result v6

    if-eqz v6, :cond_2

    .line 202
    const-string v6, "Sec-WebSocket-Accept"

    invoke-static {p1, v6, v2, v3, v2}, Lokhttp3/Response;->header$default(Lokhttp3/Response;Ljava/lang/String;Ljava/lang/String;ILjava/lang/Object;)Ljava/lang/String;

    move-result-object v2

    .line 203
    .local v2, "headerAccept":Ljava/lang/String;
    sget-object v3, Lokio/ByteString;->Companion:Lokio/ByteString$Companion;

    new-instance v6, Ljava/lang/StringBuilder;

    invoke-direct {v6}, Ljava/lang/StringBuilder;-><init>()V

    iget-object v7, p0, Lokhttp3/internal/ws/RealWebSocket;->key:Ljava/lang/String;

    invoke-virtual {v6, v7}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v7, "258EAFA5-E914-47DA-95CA-C5AB0DC85B11"

    invoke-virtual {v6, v7}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v6}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v6

    invoke-virtual {v3, v6}, Lokio/ByteString$Companion;->encodeUtf8(Ljava/lang/String;)Lokio/ByteString;

    move-result-object v3

    invoke-virtual {v3}, Lokio/ByteString;->sha1()Lokio/ByteString;

    move-result-object v3

    invoke-virtual {v3}, Lokio/ByteString;->base64()Ljava/lang/String;

    move-result-object v3

    .line 204
    .local v3, "acceptExpected":Ljava/lang/String;
    invoke-static {v3, v2}, Lkotlin/jvm/internal/Intrinsics;->areEqual(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v6

    xor-int/2addr v5, v6

    if-nez v5, :cond_1

    .line 209
    if-eqz p2, :cond_0

    .line 212
    return-void

    .line 210
    :cond_0
    new-instance v1, Ljava/net/ProtocolException;

    const-string v5, "Web Socket exchange missing: bad interceptor?"

    invoke-direct {v1, v5}, Ljava/net/ProtocolException;-><init>(Ljava/lang/String;)V

    check-cast v1, Ljava/lang/Throwable;

    throw v1

    .line 205
    :cond_1
    new-instance v5, Ljava/net/ProtocolException;

    .line 206
    new-instance v6, Ljava/lang/StringBuilder;

    invoke-direct {v6}, Ljava/lang/StringBuilder;-><init>()V

    const-string v7, "Expected \'Sec-WebSocket-Accept\' header value \'"

    invoke-virtual {v6, v7}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v6, v3}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v7, "\' but was \'"

    invoke-virtual {v6, v7}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v6, v2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v6, v1}, Ljava/lang/StringBuilder;->append(C)Ljava/lang/StringBuilder;

    invoke-virtual {v6}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v1

    .line 205
    invoke-direct {v5, v1}, Ljava/net/ProtocolException;-><init>(Ljava/lang/String;)V

    check-cast v5, Ljava/lang/Throwable;

    throw v5

    .line 198
    .end local v2    # "headerAccept":Ljava/lang/String;
    .end local v3    # "acceptExpected":Ljava/lang/String;
    :cond_2
    new-instance v2, Ljava/net/ProtocolException;

    .line 199
    new-instance v3, Ljava/lang/StringBuilder;

    invoke-direct {v3}, Ljava/lang/StringBuilder;-><init>()V

    const-string v5, "Expected \'Upgrade\' header value \'websocket\' but was \'"

    invoke-virtual {v3, v5}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v3, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v3, v1}, Ljava/lang/StringBuilder;->append(C)Ljava/lang/StringBuilder;

    invoke-virtual {v3}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v1

    .line 198
    invoke-direct {v2, v1}, Ljava/net/ProtocolException;-><init>(Ljava/lang/String;)V

    check-cast v2, Ljava/lang/Throwable;

    throw v2

    .line 192
    .end local v4    # "headerUpgrade":Ljava/lang/String;
    :cond_3
    new-instance v2, Ljava/net/ProtocolException;

    .line 193
    new-instance v3, Ljava/lang/StringBuilder;

    invoke-direct {v3}, Ljava/lang/StringBuilder;-><init>()V

    const-string v4, "Expected \'Connection\' header value \'Upgrade\' but was \'"

    invoke-virtual {v3, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v3, v0}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v3, v1}, Ljava/lang/StringBuilder;->append(C)Ljava/lang/StringBuilder;

    invoke-virtual {v3}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v1

    .line 192
    invoke-direct {v2, v1}, Ljava/net/ProtocolException;-><init>(Ljava/lang/String;)V

    check-cast v2, Ljava/lang/Throwable;

    throw v2

    .line 186
    .end local v0    # "headerConnection":Ljava/lang/String;
    :cond_4
    new-instance v0, Ljava/net/ProtocolException;

    .line 187
    new-instance v2, Ljava/lang/StringBuilder;

    invoke-direct {v2}, Ljava/lang/StringBuilder;-><init>()V

    const-string v3, "Expected HTTP 101 response but was \'"

    invoke-virtual {v2, v3}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {p1}, Lokhttp3/Response;->code()I

    move-result v3

    invoke-virtual {v2, v3}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    const/16 v3, 0x20

    invoke-virtual {v2, v3}, Ljava/lang/StringBuilder;->append(C)Ljava/lang/StringBuilder;

    invoke-virtual {p1}, Lokhttp3/Response;->message()Ljava/lang/String;

    move-result-object v3

    invoke-virtual {v2, v3}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v2, v1}, Ljava/lang/StringBuilder;->append(C)Ljava/lang/StringBuilder;

    invoke-virtual {v2}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v1

    .line 186
    invoke-direct {v0, v1}, Ljava/net/ProtocolException;-><init>(Ljava/lang/String;)V

    check-cast v0, Ljava/lang/Throwable;

    throw v0
.end method

.method public close(ILjava/lang/String;)Z
    .locals 2
    .param p1, "code"    # I
    .param p2, "reason"    # Ljava/lang/String;

    .line 367
    const-wide/32 v0, 0xea60

    invoke-virtual {p0, p1, p2, v0, v1}, Lokhttp3/internal/ws/RealWebSocket;->close(ILjava/lang/String;J)Z

    move-result v0

    return v0
.end method

.method public final declared-synchronized close(ILjava/lang/String;J)Z
    .locals 8
    .param p1, "code"    # I
    .param p2, "reason"    # Ljava/lang/String;
    .param p3, "cancelAfterCloseMillis"    # J

    monitor-enter p0

    .line 375
    :try_start_0
    sget-object v0, Lokhttp3/internal/ws/WebSocketProtocol;->INSTANCE:Lokhttp3/internal/ws/WebSocketProtocol;

    invoke-virtual {v0, p1}, Lokhttp3/internal/ws/WebSocketProtocol;->validateCloseCode(I)V

    .line 377
    const/4 v0, 0x0

    check-cast v0, Lokio/ByteString;

    .line 378
    .local v0, "reasonBytes":Lokio/ByteString;
    const/4 v1, 0x0

    const/4 v2, 0x1

    if-eqz p2, :cond_2

    .line 379
    sget-object v3, Lokio/ByteString;->Companion:Lokio/ByteString$Companion;

    invoke-virtual {v3, p2}, Lokio/ByteString$Companion;->encodeUtf8(Ljava/lang/String;)Lokio/ByteString;

    move-result-object v3

    move-object v0, v3

    .line 380
    invoke-virtual {v0}, Lokio/ByteString;->size()I

    move-result v3

    int-to-long v3, v3

    const-wide/16 v5, 0x7b

    cmp-long v7, v3, v5

    if-gtz v7, :cond_0

    const/4 v3, 0x1

    goto :goto_0

    :cond_0
    const/4 v3, 0x0

    :goto_0
    if-eqz v3, :cond_1

    goto :goto_1

    :cond_1
    const/4 v1, 0x0

    .line 381
    .local v1, "$i$a$-require-RealWebSocket$close$1":I
    new-instance v2, Ljava/lang/StringBuilder;

    invoke-direct {v2}, Ljava/lang/StringBuilder;-><init>()V

    const-string v3, "reason.size() > 123: "

    invoke-virtual {v2, v3}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v2, p2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v2}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v2

    .line 380
    .end local v1    # "$i$a$-require-RealWebSocket$close$1":I
    new-instance v1, Ljava/lang/IllegalArgumentException;

    invoke-virtual {v2}, Ljava/lang/Object;->toString()Ljava/lang/String;

    move-result-object v2

    invoke-direct {v1, v2}, Ljava/lang/IllegalArgumentException;-><init>(Ljava/lang/String;)V

    check-cast v1, Ljava/lang/Throwable;

    throw v1

    .line 385
    .end local p0    # "this":Lokhttp3/internal/ws/RealWebSocket;
    :cond_2
    :goto_1
    iget-boolean v3, p0, Lokhttp3/internal/ws/RealWebSocket;->failed:Z

    if-nez v3, :cond_4

    iget-boolean v3, p0, Lokhttp3/internal/ws/RealWebSocket;->enqueuedClose:Z

    if-eqz v3, :cond_3

    goto :goto_2

    .line 388
    :cond_3
    iput-boolean v2, p0, Lokhttp3/internal/ws/RealWebSocket;->enqueuedClose:Z

    .line 391
    iget-object v1, p0, Lokhttp3/internal/ws/RealWebSocket;->messageAndCloseQueue:Ljava/util/ArrayDeque;

    new-instance v3, Lokhttp3/internal/ws/RealWebSocket$Close;

    invoke-direct {v3, p1, v0, p3, p4}, Lokhttp3/internal/ws/RealWebSocket$Close;-><init>(ILokio/ByteString;J)V

    invoke-virtual {v1, v3}, Ljava/util/ArrayDeque;->add(Ljava/lang/Object;)Z

    .line 392
    invoke-direct {p0}, Lokhttp3/internal/ws/RealWebSocket;->runWriter()V
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    .line 393
    monitor-exit p0

    return v2

    .line 385
    :cond_4
    :goto_2
    monitor-exit p0

    return v1

    .line 374
    .end local v0    # "reasonBytes":Lokio/ByteString;
    .end local p1    # "code":I
    .end local p2    # "reason":Ljava/lang/String;
    .end local p3    # "cancelAfterCloseMillis":J
    :catchall_0
    move-exception p1

    monitor-exit p0

    throw p1
.end method

.method public final connect(Lokhttp3/OkHttpClient;)V
    .locals 4
    .param p1, "client"    # Lokhttp3/OkHttpClient;

    const-string v0, "client"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkParameterIsNotNull(Ljava/lang/Object;Ljava/lang/String;)V

    .line 141
    nop

    .line 144
    nop

    .line 141
    nop

    .line 143
    nop

    .line 141
    nop

    .line 142
    nop

    .line 141
    invoke-virtual {p1}, Lokhttp3/OkHttpClient;->newBuilder()Lokhttp3/OkHttpClient$Builder;

    move-result-object v0

    .line 142
    sget-object v1, Lokhttp3/EventListener;->NONE:Lokhttp3/EventListener;

    invoke-virtual {v0, v1}, Lokhttp3/OkHttpClient$Builder;->eventListener(Lokhttp3/EventListener;)Lokhttp3/OkHttpClient$Builder;

    move-result-object v0

    .line 143
    sget-object v1, Lokhttp3/internal/ws/RealWebSocket;->ONLY_HTTP1:Ljava/util/List;

    invoke-virtual {v0, v1}, Lokhttp3/OkHttpClient$Builder;->protocols(Ljava/util/List;)Lokhttp3/OkHttpClient$Builder;

    move-result-object v0

    .line 144
    invoke-virtual {v0}, Lokhttp3/OkHttpClient$Builder;->build()Lokhttp3/OkHttpClient;

    move-result-object v0

    .line 141
    nop

    .line 145
    .local v0, "webSocketClient":Lokhttp3/OkHttpClient;
    nop

    .line 150
    nop

    .line 145
    nop

    .line 149
    nop

    .line 145
    nop

    .line 148
    nop

    .line 145
    nop

    .line 147
    nop

    .line 145
    nop

    .line 146
    nop

    .line 145
    iget-object v1, p0, Lokhttp3/internal/ws/RealWebSocket;->originalRequest:Lokhttp3/Request;

    invoke-virtual {v1}, Lokhttp3/Request;->newBuilder()Lokhttp3/Request$Builder;

    move-result-object v1

    .line 146
    const-string v2, "Upgrade"

    const-string v3, "websocket"

    invoke-virtual {v1, v2, v3}, Lokhttp3/Request$Builder;->header(Ljava/lang/String;Ljava/lang/String;)Lokhttp3/Request$Builder;

    move-result-object v1

    .line 147
    const-string v3, "Connection"

    invoke-virtual {v1, v3, v2}, Lokhttp3/Request$Builder;->header(Ljava/lang/String;Ljava/lang/String;)Lokhttp3/Request$Builder;

    move-result-object v1

    .line 148
    iget-object v2, p0, Lokhttp3/internal/ws/RealWebSocket;->key:Ljava/lang/String;

    const-string v3, "Sec-WebSocket-Key"

    invoke-virtual {v1, v3, v2}, Lokhttp3/Request$Builder;->header(Ljava/lang/String;Ljava/lang/String;)Lokhttp3/Request$Builder;

    move-result-object v1

    .line 149
    const-string v2, "Sec-WebSocket-Version"

    const-string v3, "13"

    invoke-virtual {v1, v2, v3}, Lokhttp3/Request$Builder;->header(Ljava/lang/String;Ljava/lang/String;)Lokhttp3/Request$Builder;

    move-result-object v1

    .line 150
    invoke-virtual {v1}, Lokhttp3/Request$Builder;->build()Lokhttp3/Request;

    move-result-object v1

    .line 145
    nop

    .line 151
    .local v1, "request":Lokhttp3/Request;
    new-instance v2, Lokhttp3/internal/connection/RealCall;

    const/4 v3, 0x1

    invoke-direct {v2, v0, v1, v3}, Lokhttp3/internal/connection/RealCall;-><init>(Lokhttp3/OkHttpClient;Lokhttp3/Request;Z)V

    check-cast v2, Lokhttp3/Call;

    iput-object v2, p0, Lokhttp3/internal/ws/RealWebSocket;->call:Lokhttp3/Call;

    .line 152
    if-nez v2, :cond_0

    invoke-static {}, Lkotlin/jvm/internal/Intrinsics;->throwNpe()V

    :cond_0
    new-instance v3, Lokhttp3/internal/ws/RealWebSocket$connect$1;

    invoke-direct {v3, p0, v1}, Lokhttp3/internal/ws/RealWebSocket$connect$1;-><init>(Lokhttp3/internal/ws/RealWebSocket;Lokhttp3/Request;)V

    check-cast v3, Lokhttp3/Callback;

    invoke-interface {v2, v3}, Lokhttp3/Call;->enqueue(Lokhttp3/Callback;)V

    .line 181
    return-void
.end method

.method public final failWebSocket(Ljava/lang/Exception;Lokhttp3/Response;)V
    .locals 4
    .param p1, "e"    # Ljava/lang/Exception;
    .param p2, "response"    # Lokhttp3/Response;

    const-string v0, "e"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkParameterIsNotNull(Ljava/lang/Object;Ljava/lang/String;)V

    .line 511
    const/4 v0, 0x0

    .line 512
    .local v0, "streamsToClose":Lokhttp3/internal/ws/RealWebSocket$Streams;
    monitor-enter p0

    const/4 v1, 0x0

    .line 513
    .local v1, "$i$a$-synchronized-RealWebSocket$failWebSocket$1":I
    :try_start_0
    iget-boolean v2, p0, Lokhttp3/internal/ws/RealWebSocket;->failed:Z
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_1

    if-eqz v2, :cond_0

    .end local v1    # "$i$a$-synchronized-RealWebSocket$failWebSocket$1":I
    monitor-exit p0

    return-void

    .line 514
    .restart local v1    # "$i$a$-synchronized-RealWebSocket$failWebSocket$1":I
    :cond_0
    const/4 v2, 0x1

    :try_start_1
    iput-boolean v2, p0, Lokhttp3/internal/ws/RealWebSocket;->failed:Z

    .line 515
    iget-object v2, p0, Lokhttp3/internal/ws/RealWebSocket;->streams:Lokhttp3/internal/ws/RealWebSocket$Streams;

    move-object v0, v2

    .line 516
    const/4 v2, 0x0

    check-cast v2, Lokhttp3/internal/ws/RealWebSocket$Streams;

    iput-object v2, p0, Lokhttp3/internal/ws/RealWebSocket;->streams:Lokhttp3/internal/ws/RealWebSocket$Streams;

    .line 517
    iget-object v2, p0, Lokhttp3/internal/ws/RealWebSocket;->taskQueue:Lokhttp3/internal/concurrent/TaskQueue;

    invoke-virtual {v2}, Lokhttp3/internal/concurrent/TaskQueue;->shutdown()V

    .line 518
    .end local v1    # "$i$a$-synchronized-RealWebSocket$failWebSocket$1":I
    sget-object v1, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;
    :try_end_1
    .catchall {:try_start_1 .. :try_end_1} :catchall_1

    .line 512
    monitor-exit p0

    .line 520
    nop

    .line 521
    :try_start_2
    iget-object v1, p0, Lokhttp3/internal/ws/RealWebSocket;->listener:Lokhttp3/WebSocketListener;

    move-object v2, p0

    check-cast v2, Lokhttp3/WebSocket;

    move-object v3, p1

    check-cast v3, Ljava/lang/Throwable;

    invoke-virtual {v1, v2, v3, p2}, Lokhttp3/WebSocketListener;->onFailure(Lokhttp3/WebSocket;Ljava/lang/Throwable;Lokhttp3/Response;)V
    :try_end_2
    .catchall {:try_start_2 .. :try_end_2} :catchall_0

    .line 523
    if-eqz v0, :cond_1

    move-object v1, v0

    check-cast v1, Ljava/io/Closeable;

    invoke-static {v1}, Lokhttp3/internal/Util;->closeQuietly(Ljava/io/Closeable;)V

    .line 524
    :cond_1
    nop

    .line 525
    return-void

    .line 524
    :catchall_0
    move-exception v1

    .line 523
    if-eqz v0, :cond_2

    move-object v2, v0

    check-cast v2, Ljava/io/Closeable;

    invoke-static {v2}, Lokhttp3/internal/Util;->closeQuietly(Ljava/io/Closeable;)V

    :cond_2
    throw v1

    .line 512
    :catchall_1
    move-exception v1

    monitor-exit p0

    throw v1
.end method

.method public final getListener$okhttp()Lokhttp3/WebSocketListener;
    .locals 1

    .line 58
    iget-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->listener:Lokhttp3/WebSocketListener;

    return-object v0
.end method

.method public final initReaderAndWriter(Ljava/lang/String;Lokhttp3/internal/ws/RealWebSocket$Streams;)V
    .locals 17
    .param p1, "name"    # Ljava/lang/String;
    .param p2, "streams"    # Lokhttp3/internal/ws/RealWebSocket$Streams;
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Ljava/io/IOException;
        }
    .end annotation

    move-object/from16 v9, p0

    move-object/from16 v10, p1

    move-object/from16 v11, p2

    const-string v0, "name"

    invoke-static {v10, v0}, Lkotlin/jvm/internal/Intrinsics;->checkParameterIsNotNull(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v0, "streams"

    invoke-static {v11, v0}, Lkotlin/jvm/internal/Intrinsics;->checkParameterIsNotNull(Ljava/lang/Object;Ljava/lang/String;)V

    .line 216
    monitor-enter p0

    const/4 v0, 0x0

    .line 217
    .local v0, "$i$a$-synchronized-RealWebSocket$initReaderAndWriter$1":I
    :try_start_0
    iput-object v10, v9, Lokhttp3/internal/ws/RealWebSocket;->name:Ljava/lang/String;

    .line 218
    iput-object v11, v9, Lokhttp3/internal/ws/RealWebSocket;->streams:Lokhttp3/internal/ws/RealWebSocket$Streams;

    .line 219
    new-instance v1, Lokhttp3/internal/ws/WebSocketWriter;

    invoke-virtual/range {p2 .. p2}, Lokhttp3/internal/ws/RealWebSocket$Streams;->getClient()Z

    move-result v2

    invoke-virtual/range {p2 .. p2}, Lokhttp3/internal/ws/RealWebSocket$Streams;->getSink()Lokio/BufferedSink;

    move-result-object v3

    iget-object v4, v9, Lokhttp3/internal/ws/RealWebSocket;->random:Ljava/util/Random;

    invoke-direct {v1, v2, v3, v4}, Lokhttp3/internal/ws/WebSocketWriter;-><init>(ZLokio/BufferedSink;Ljava/util/Random;)V

    iput-object v1, v9, Lokhttp3/internal/ws/RealWebSocket;->writer:Lokhttp3/internal/ws/WebSocketWriter;

    .line 220
    new-instance v1, Lokhttp3/internal/ws/RealWebSocket$WriterTask;

    invoke-direct {v1, v9}, Lokhttp3/internal/ws/RealWebSocket$WriterTask;-><init>(Lokhttp3/internal/ws/RealWebSocket;)V

    check-cast v1, Lokhttp3/internal/concurrent/Task;

    iput-object v1, v9, Lokhttp3/internal/ws/RealWebSocket;->writerTask:Lokhttp3/internal/concurrent/Task;

    .line 221
    iget-wide v1, v9, Lokhttp3/internal/ws/RealWebSocket;->pingIntervalMillis:J

    const-wide/16 v3, 0x0

    cmp-long v5, v1, v3

    if-eqz v5, :cond_0

    .line 222
    sget-object v1, Ljava/util/concurrent/TimeUnit;->MILLISECONDS:Ljava/util/concurrent/TimeUnit;

    iget-wide v2, v9, Lokhttp3/internal/ws/RealWebSocket;->pingIntervalMillis:J

    invoke-virtual {v1, v2, v3}, Ljava/util/concurrent/TimeUnit;->toNanos(J)J

    move-result-wide v1

    move-wide v12, v1

    .line 223
    .local v12, "pingIntervalNanos":J
    iget-object v1, v9, Lokhttp3/internal/ws/RealWebSocket;->taskQueue:Lokhttp3/internal/concurrent/TaskQueue;

    new-instance v2, Ljava/lang/StringBuilder;

    invoke-direct {v2}, Ljava/lang/StringBuilder;-><init>()V

    invoke-virtual {v2, v10}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v3, " ping"

    invoke-virtual {v2, v3}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v2}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v2

    .local v2, "name$iv":Ljava/lang/String;
    move-object v14, v1

    .local v14, "this_$iv":Lokhttp3/internal/concurrent/TaskQueue;
    const/4 v15, 0x0

    .line 572
    .local v15, "$i$f$schedule":I
    new-instance v16, Lokhttp3/internal/ws/RealWebSocket$initReaderAndWriter$$inlined$synchronized$lambda$1;

    move-object/from16 v1, v16

    move-object v3, v2

    move-wide v4, v12

    move-object/from16 v6, p0

    move-object/from16 v7, p1

    move-object/from16 v8, p2

    invoke-direct/range {v1 .. v8}, Lokhttp3/internal/ws/RealWebSocket$initReaderAndWriter$$inlined$synchronized$lambda$1;-><init>(Ljava/lang/String;Ljava/lang/String;JLokhttp3/internal/ws/RealWebSocket;Ljava/lang/String;Lokhttp3/internal/ws/RealWebSocket$Streams;)V

    move-object/from16 v1, v16

    check-cast v1, Lokhttp3/internal/concurrent/Task;

    .line 574
    nop

    .line 572
    invoke-virtual {v14, v1, v12, v13}, Lokhttp3/internal/concurrent/TaskQueue;->schedule(Lokhttp3/internal/concurrent/Task;J)V

    .line 228
    .end local v2    # "name$iv":Ljava/lang/String;
    .end local v12    # "pingIntervalNanos":J
    .end local v14    # "this_$iv":Lokhttp3/internal/concurrent/TaskQueue;
    .end local v15    # "$i$f$schedule":I
    :cond_0
    iget-object v1, v9, Lokhttp3/internal/ws/RealWebSocket;->messageAndCloseQueue:Ljava/util/ArrayDeque;

    check-cast v1, Ljava/util/Collection;

    invoke-interface {v1}, Ljava/util/Collection;->isEmpty()Z

    move-result v1

    xor-int/lit8 v1, v1, 0x1

    if-eqz v1, :cond_1

    .line 229
    invoke-direct/range {p0 .. p0}, Lokhttp3/internal/ws/RealWebSocket;->runWriter()V

    .line 231
    .end local v0    # "$i$a$-synchronized-RealWebSocket$initReaderAndWriter$1":I
    :cond_1
    sget-object v0, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    .line 216
    monitor-exit p0

    .line 233
    new-instance v0, Lokhttp3/internal/ws/WebSocketReader;

    invoke-virtual/range {p2 .. p2}, Lokhttp3/internal/ws/RealWebSocket$Streams;->getClient()Z

    move-result v1

    invoke-virtual/range {p2 .. p2}, Lokhttp3/internal/ws/RealWebSocket$Streams;->getSource()Lokio/BufferedSource;

    move-result-object v2

    move-object v3, v9

    check-cast v3, Lokhttp3/internal/ws/WebSocketReader$FrameCallback;

    invoke-direct {v0, v1, v2, v3}, Lokhttp3/internal/ws/WebSocketReader;-><init>(ZLokio/BufferedSource;Lokhttp3/internal/ws/WebSocketReader$FrameCallback;)V

    iput-object v0, v9, Lokhttp3/internal/ws/RealWebSocket;->reader:Lokhttp3/internal/ws/WebSocketReader;

    .line 234
    return-void

    .line 216
    :catchall_0
    move-exception v0

    monitor-exit p0

    throw v0
.end method

.method public final loopReader()V
    .locals 2
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Ljava/io/IOException;
        }
    .end annotation

    .line 239
    :goto_0
    iget v0, p0, Lokhttp3/internal/ws/RealWebSocket;->receivedCloseCode:I

    const/4 v1, -0x1

    if-ne v0, v1, :cond_1

    .line 241
    iget-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->reader:Lokhttp3/internal/ws/WebSocketReader;

    if-nez v0, :cond_0

    invoke-static {}, Lkotlin/jvm/internal/Intrinsics;->throwNpe()V

    :cond_0
    invoke-virtual {v0}, Lokhttp3/internal/ws/WebSocketReader;->processNextFrame()V

    .line 239
    goto :goto_0

    .line 243
    :cond_1
    return-void
.end method

.method public onReadClose(ILjava/lang/String;)V
    .locals 7
    .param p1, "code"    # I
    .param p2, "reason"    # Ljava/lang/String;

    const-string v0, "reason"

    invoke-static {p2, v0}, Lkotlin/jvm/internal/Intrinsics;->checkParameterIsNotNull(Ljava/lang/Object;Ljava/lang/String;)V

    .line 305
    const/4 v0, 0x1

    const/4 v1, 0x0

    const/4 v2, -0x1

    if-eq p1, v2, :cond_0

    const/4 v3, 0x1

    goto :goto_0

    :cond_0
    const/4 v3, 0x0

    :goto_0
    if-eqz v3, :cond_7

    .line 307
    const/4 v3, 0x0

    move-object v4, v3

    check-cast v4, Lokhttp3/internal/ws/RealWebSocket$Streams;

    .line 308
    .local v4, "toClose":Ljava/lang/Object;
    monitor-enter p0

    const/4 v5, 0x0

    .line 309
    .local v5, "$i$a$-synchronized-RealWebSocket$onReadClose$1":I
    :try_start_0
    iget v6, p0, Lokhttp3/internal/ws/RealWebSocket;->receivedCloseCode:I

    if-ne v6, v2, :cond_1

    goto :goto_1

    :cond_1
    const/4 v0, 0x0

    :goto_1
    if-eqz v0, :cond_6

    .line 310
    iput p1, p0, Lokhttp3/internal/ws/RealWebSocket;->receivedCloseCode:I

    .line 311
    iput-object p2, p0, Lokhttp3/internal/ws/RealWebSocket;->receivedCloseReason:Ljava/lang/String;

    .line 312
    iget-boolean v0, p0, Lokhttp3/internal/ws/RealWebSocket;->enqueuedClose:Z

    if-eqz v0, :cond_2

    iget-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->messageAndCloseQueue:Ljava/util/ArrayDeque;

    invoke-virtual {v0}, Ljava/util/ArrayDeque;->isEmpty()Z

    move-result v0

    if-eqz v0, :cond_2

    .line 313
    iget-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->streams:Lokhttp3/internal/ws/RealWebSocket$Streams;

    move-object v4, v0

    .line 314
    check-cast v3, Lokhttp3/internal/ws/RealWebSocket$Streams;

    iput-object v3, p0, Lokhttp3/internal/ws/RealWebSocket;->streams:Lokhttp3/internal/ws/RealWebSocket$Streams;

    .line 315
    iget-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->taskQueue:Lokhttp3/internal/concurrent/TaskQueue;

    invoke-virtual {v0}, Lokhttp3/internal/concurrent/TaskQueue;->shutdown()V

    .line 317
    .end local v5    # "$i$a$-synchronized-RealWebSocket$onReadClose$1":I
    :cond_2
    sget-object v0, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_1

    .line 308
    monitor-exit p0

    .line 319
    nop

    .line 320
    :try_start_1
    iget-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->listener:Lokhttp3/WebSocketListener;

    move-object v1, p0

    check-cast v1, Lokhttp3/WebSocket;

    invoke-virtual {v0, v1, p1, p2}, Lokhttp3/WebSocketListener;->onClosing(Lokhttp3/WebSocket;ILjava/lang/String;)V

    .line 322
    if-eqz v4, :cond_3

    .line 323
    iget-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->listener:Lokhttp3/WebSocketListener;

    move-object v1, p0

    check-cast v1, Lokhttp3/WebSocket;

    invoke-virtual {v0, v1, p1, p2}, Lokhttp3/WebSocketListener;->onClosed(Lokhttp3/WebSocket;ILjava/lang/String;)V
    :try_end_1
    .catchall {:try_start_1 .. :try_end_1} :catchall_0

    .line 326
    :cond_3
    if-eqz v4, :cond_4

    move-object v0, v4

    check-cast v0, Ljava/io/Closeable;

    invoke-static {v0}, Lokhttp3/internal/Util;->closeQuietly(Ljava/io/Closeable;)V

    .line 327
    :cond_4
    nop

    .line 328
    return-void

    .line 327
    :catchall_0
    move-exception v0

    .line 326
    if-eqz v4, :cond_5

    move-object v1, v4

    check-cast v1, Ljava/io/Closeable;

    invoke-static {v1}, Lokhttp3/internal/Util;->closeQuietly(Ljava/io/Closeable;)V

    :cond_5
    throw v0

    .line 309
    .restart local v5    # "$i$a$-synchronized-RealWebSocket$onReadClose$1":I
    :cond_6
    const/4 v0, 0x0

    .local v0, "$i$a$-check-RealWebSocket$onReadClose$1$1":I
    :try_start_2
    const-string v1, "already closed"

    .end local v0    # "$i$a$-check-RealWebSocket$onReadClose$1$1":I
    new-instance v0, Ljava/lang/IllegalStateException;

    invoke-virtual {v1}, Ljava/lang/Object;->toString()Ljava/lang/String;

    move-result-object v1

    invoke-direct {v0, v1}, Ljava/lang/IllegalStateException;-><init>(Ljava/lang/String;)V

    check-cast v0, Ljava/lang/Throwable;

    .end local v4    # "toClose":Ljava/lang/Object;
    .end local p1    # "code":I
    .end local p2    # "reason":Ljava/lang/String;
    throw v0
    :try_end_2
    .catchall {:try_start_2 .. :try_end_2} :catchall_1

    .line 308
    .end local v5    # "$i$a$-synchronized-RealWebSocket$onReadClose$1":I
    .restart local v4    # "toClose":Ljava/lang/Object;
    .restart local p1    # "code":I
    .restart local p2    # "reason":Ljava/lang/String;
    :catchall_1
    move-exception v0

    monitor-exit p0

    throw v0

    .line 305
    .end local v4    # "toClose":Ljava/lang/Object;
    :cond_7
    const-string v0, "Failed requirement."

    new-instance v1, Ljava/lang/IllegalArgumentException;

    invoke-virtual {v0}, Ljava/lang/Object;->toString()Ljava/lang/String;

    move-result-object v0

    invoke-direct {v1, v0}, Ljava/lang/IllegalArgumentException;-><init>(Ljava/lang/String;)V

    check-cast v1, Ljava/lang/Throwable;

    throw v1
.end method

.method public onReadMessage(Ljava/lang/String;)V
    .locals 2
    .param p1, "text"    # Ljava/lang/String;
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Ljava/io/IOException;
        }
    .end annotation

    const-string v0, "text"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkParameterIsNotNull(Ljava/lang/Object;Ljava/lang/String;)V

    .line 281
    iget-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->listener:Lokhttp3/WebSocketListener;

    move-object v1, p0

    check-cast v1, Lokhttp3/WebSocket;

    invoke-virtual {v0, v1, p1}, Lokhttp3/WebSocketListener;->onMessage(Lokhttp3/WebSocket;Ljava/lang/String;)V

    .line 282
    return-void
.end method

.method public onReadMessage(Lokio/ByteString;)V
    .locals 2
    .param p1, "bytes"    # Lokio/ByteString;
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Ljava/io/IOException;
        }
    .end annotation

    const-string v0, "bytes"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkParameterIsNotNull(Ljava/lang/Object;Ljava/lang/String;)V

    .line 286
    iget-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->listener:Lokhttp3/WebSocketListener;

    move-object v1, p0

    check-cast v1, Lokhttp3/WebSocket;

    invoke-virtual {v0, v1, p1}, Lokhttp3/WebSocketListener;->onMessage(Lokhttp3/WebSocket;Lokio/ByteString;)V

    .line 287
    return-void
.end method

.method public declared-synchronized onReadPing(Lokio/ByteString;)V
    .locals 1
    .param p1, "payload"    # Lokio/ByteString;

    monitor-enter p0

    :try_start_0
    const-string v0, "payload"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkParameterIsNotNull(Ljava/lang/Object;Ljava/lang/String;)V

    .line 291
    iget-boolean v0, p0, Lokhttp3/internal/ws/RealWebSocket;->failed:Z

    if-nez v0, :cond_1

    iget-boolean v0, p0, Lokhttp3/internal/ws/RealWebSocket;->enqueuedClose:Z

    if-eqz v0, :cond_0

    iget-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->messageAndCloseQueue:Ljava/util/ArrayDeque;

    invoke-virtual {v0}, Ljava/util/ArrayDeque;->isEmpty()Z

    move-result v0

    if-eqz v0, :cond_0

    goto :goto_0

    .line 293
    .end local p0    # "this":Lokhttp3/internal/ws/RealWebSocket;
    :cond_0
    iget-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->pongQueue:Ljava/util/ArrayDeque;

    invoke-virtual {v0, p1}, Ljava/util/ArrayDeque;->add(Ljava/lang/Object;)Z

    .line 294
    invoke-direct {p0}, Lokhttp3/internal/ws/RealWebSocket;->runWriter()V

    .line 295
    iget v0, p0, Lokhttp3/internal/ws/RealWebSocket;->receivedPingCount:I

    add-int/lit8 v0, v0, 0x1

    iput v0, p0, Lokhttp3/internal/ws/RealWebSocket;->receivedPingCount:I
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    .line 296
    monitor-exit p0

    return-void

    .line 291
    :cond_1
    :goto_0
    monitor-exit p0

    return-void

    .line 290
    .end local p1    # "payload":Lokio/ByteString;
    :catchall_0
    move-exception p1

    monitor-exit p0

    throw p1
.end method

.method public declared-synchronized onReadPong(Lokio/ByteString;)V
    .locals 1
    .param p1, "payload"    # Lokio/ByteString;

    monitor-enter p0

    :try_start_0
    const-string v0, "payload"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkParameterIsNotNull(Ljava/lang/Object;Ljava/lang/String;)V

    .line 300
    iget v0, p0, Lokhttp3/internal/ws/RealWebSocket;->receivedPongCount:I

    add-int/lit8 v0, v0, 0x1

    iput v0, p0, Lokhttp3/internal/ws/RealWebSocket;->receivedPongCount:I

    .line 301
    const/4 v0, 0x0

    iput-boolean v0, p0, Lokhttp3/internal/ws/RealWebSocket;->awaitingPong:Z
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    .line 302
    monitor-exit p0

    return-void

    .line 299
    .end local p0    # "this":Lokhttp3/internal/ws/RealWebSocket;
    .end local p1    # "payload":Lokio/ByteString;
    :catchall_0
    move-exception p1

    monitor-exit p0

    throw p1
.end method

.method public final declared-synchronized pong(Lokio/ByteString;)Z
    .locals 1
    .param p1, "payload"    # Lokio/ByteString;

    monitor-enter p0

    :try_start_0
    const-string v0, "payload"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkParameterIsNotNull(Ljava/lang/Object;Ljava/lang/String;)V

    .line 359
    iget-boolean v0, p0, Lokhttp3/internal/ws/RealWebSocket;->failed:Z

    if-nez v0, :cond_1

    iget-boolean v0, p0, Lokhttp3/internal/ws/RealWebSocket;->enqueuedClose:Z

    if-eqz v0, :cond_0

    iget-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->messageAndCloseQueue:Ljava/util/ArrayDeque;

    invoke-virtual {v0}, Ljava/util/ArrayDeque;->isEmpty()Z

    move-result v0

    if-eqz v0, :cond_0

    goto :goto_0

    .line 361
    .end local p0    # "this":Lokhttp3/internal/ws/RealWebSocket;
    :cond_0
    iget-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->pongQueue:Ljava/util/ArrayDeque;

    invoke-virtual {v0, p1}, Ljava/util/ArrayDeque;->add(Ljava/lang/Object;)Z

    .line 362
    invoke-direct {p0}, Lokhttp3/internal/ws/RealWebSocket;->runWriter()V
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    .line 363
    const/4 v0, 0x1

    monitor-exit p0

    return v0

    .line 359
    :cond_1
    :goto_0
    const/4 v0, 0x0

    monitor-exit p0

    return v0

    .line 358
    .end local p1    # "payload":Lokio/ByteString;
    :catchall_0
    move-exception p1

    monitor-exit p0

    throw p1
.end method

.method public final processNextFrame()Z
    .locals 3
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Ljava/io/IOException;
        }
    .end annotation

    .line 251
    nop

    .line 252
    const/4 v0, 0x0

    :try_start_0
    iget-object v1, p0, Lokhttp3/internal/ws/RealWebSocket;->reader:Lokhttp3/internal/ws/WebSocketReader;

    if-nez v1, :cond_0

    invoke-static {}, Lkotlin/jvm/internal/Intrinsics;->throwNpe()V

    :cond_0
    invoke-virtual {v1}, Lokhttp3/internal/ws/WebSocketReader;->processNextFrame()V

    .line 253
    iget v1, p0, Lokhttp3/internal/ws/RealWebSocket;->receivedCloseCode:I
    :try_end_0
    .catch Ljava/lang/Exception; {:try_start_0 .. :try_end_0} :catch_0

    const/4 v2, -0x1

    if-ne v1, v2, :cond_1

    const/4 v0, 0x1

    goto :goto_0

    .line 254
    :catch_0
    move-exception v1

    .line 255
    .local v1, "e":Ljava/lang/Exception;
    const/4 v2, 0x0

    invoke-virtual {p0, v1, v2}, Lokhttp3/internal/ws/RealWebSocket;->failWebSocket(Ljava/lang/Exception;Lokhttp3/Response;)V

    .line 256
    nop

    .line 251
    .end local v1    # "e":Ljava/lang/Exception;
    :cond_1
    :goto_0
    return v0
.end method

.method public declared-synchronized queueSize()J
    .locals 2

    monitor-enter p0

    .line 134
    :try_start_0
    iget-wide v0, p0, Lokhttp3/internal/ws/RealWebSocket;->queueSize:J
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    monitor-exit p0

    return-wide v0

    .line 134
    .end local p0    # "this":Lokhttp3/internal/ws/RealWebSocket;
    :catchall_0
    move-exception v0

    monitor-exit p0

    throw v0
.end method

.method public final declared-synchronized receivedPingCount()I
    .locals 1

    monitor-enter p0

    .line 275
    :try_start_0
    iget v0, p0, Lokhttp3/internal/ws/RealWebSocket;->receivedPingCount:I
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    monitor-exit p0

    return v0

    .line 275
    .end local p0    # "this":Lokhttp3/internal/ws/RealWebSocket;
    :catchall_0
    move-exception v0

    monitor-exit p0

    throw v0
.end method

.method public final declared-synchronized receivedPongCount()I
    .locals 1

    monitor-enter p0

    .line 277
    :try_start_0
    iget v0, p0, Lokhttp3/internal/ws/RealWebSocket;->receivedPongCount:I
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    monitor-exit p0

    return v0

    .line 277
    .end local p0    # "this":Lokhttp3/internal/ws/RealWebSocket;
    :catchall_0
    move-exception v0

    monitor-exit p0

    throw v0
.end method

.method public request()Lokhttp3/Request;
    .locals 1

    .line 132
    iget-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->originalRequest:Lokhttp3/Request;

    return-object v0
.end method

.method public send(Ljava/lang/String;)Z
    .locals 2
    .param p1, "text"    # Ljava/lang/String;

    const-string v0, "text"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkParameterIsNotNull(Ljava/lang/Object;Ljava/lang/String;)V

    .line 333
    sget-object v0, Lokio/ByteString;->Companion:Lokio/ByteString$Companion;

    invoke-virtual {v0, p1}, Lokio/ByteString$Companion;->encodeUtf8(Ljava/lang/String;)Lokio/ByteString;

    move-result-object v0

    const/4 v1, 0x1

    invoke-direct {p0, v0, v1}, Lokhttp3/internal/ws/RealWebSocket;->send(Lokio/ByteString;I)Z

    move-result v0

    return v0
.end method

.method public send(Lokio/ByteString;)Z
    .locals 1
    .param p1, "bytes"    # Lokio/ByteString;

    const-string v0, "bytes"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkParameterIsNotNull(Ljava/lang/Object;Ljava/lang/String;)V

    .line 337
    const/4 v0, 0x2

    invoke-direct {p0, p1, v0}, Lokhttp3/internal/ws/RealWebSocket;->send(Lokio/ByteString;I)Z

    move-result v0

    return v0
.end method

.method public final declared-synchronized sentPingCount()I
    .locals 1

    monitor-enter p0

    .line 273
    :try_start_0
    iget v0, p0, Lokhttp3/internal/ws/RealWebSocket;->sentPingCount:I
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    monitor-exit p0

    return v0

    .line 273
    .end local p0    # "this":Lokhttp3/internal/ws/RealWebSocket;
    :catchall_0
    move-exception v0

    monitor-exit p0

    throw v0
.end method

.method public final tearDown()V
    .locals 4
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Ljava/lang/InterruptedException;
        }
    .end annotation

    .line 269
    iget-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->taskQueue:Lokhttp3/internal/concurrent/TaskQueue;

    invoke-virtual {v0}, Lokhttp3/internal/concurrent/TaskQueue;->shutdown()V

    .line 270
    iget-object v0, p0, Lokhttp3/internal/ws/RealWebSocket;->taskQueue:Lokhttp3/internal/concurrent/TaskQueue;

    invoke-virtual {v0}, Lokhttp3/internal/concurrent/TaskQueue;->idleLatch()Ljava/util/concurrent/CountDownLatch;

    move-result-object v0

    sget-object v1, Ljava/util/concurrent/TimeUnit;->SECONDS:Ljava/util/concurrent/TimeUnit;

    const-wide/16 v2, 0xa

    invoke-virtual {v0, v2, v3, v1}, Ljava/util/concurrent/CountDownLatch;->await(JLjava/util/concurrent/TimeUnit;)Z

    .line 271
    return-void
.end method

.method public final writeOneFrame$okhttp()Z
    .locals 28
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Ljava/io/IOException;
        }
    .end annotation

    .line 420
    move-object/from16 v13, p0

    const/4 v1, 0x0

    .line 421
    .local v1, "writer":Lokhttp3/internal/ws/WebSocketWriter;
    const/4 v2, 0x0

    .line 422
    .local v2, "pong":Lokio/ByteString;
    new-instance v0, Lkotlin/jvm/internal/Ref$ObjectRef;

    invoke-direct {v0}, Lkotlin/jvm/internal/Ref$ObjectRef;-><init>()V

    const/4 v3, 0x0

    iput-object v3, v0, Lkotlin/jvm/internal/Ref$ObjectRef;->element:Ljava/lang/Object;

    move-object v14, v0

    .line 423
    .local v14, "messageOrClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    new-instance v0, Lkotlin/jvm/internal/Ref$IntRef;

    invoke-direct {v0}, Lkotlin/jvm/internal/Ref$IntRef;-><init>()V

    const/4 v4, -0x1

    iput v4, v0, Lkotlin/jvm/internal/Ref$IntRef;->element:I

    move-object v15, v0

    .line 424
    .local v15, "receivedCloseCode":Lkotlin/jvm/internal/Ref$IntRef;
    new-instance v0, Lkotlin/jvm/internal/Ref$ObjectRef;

    invoke-direct {v0}, Lkotlin/jvm/internal/Ref$ObjectRef;-><init>()V

    move-object v5, v3

    check-cast v5, Ljava/lang/String;

    iput-object v5, v0, Lkotlin/jvm/internal/Ref$ObjectRef;->element:Ljava/lang/Object;

    move-object v12, v0

    .line 425
    .local v12, "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    new-instance v0, Lkotlin/jvm/internal/Ref$ObjectRef;

    invoke-direct {v0}, Lkotlin/jvm/internal/Ref$ObjectRef;-><init>()V

    move-object v5, v3

    check-cast v5, Lokhttp3/internal/ws/RealWebSocket$Streams;

    iput-object v5, v0, Lkotlin/jvm/internal/Ref$ObjectRef;->element:Ljava/lang/Object;

    move-object v11, v0

    .line 427
    .local v11, "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    monitor-enter p0

    const/4 v0, 0x0

    .line 428
    .local v0, "$i$a$-synchronized-RealWebSocket$writeOneFrame$1":I
    :try_start_0
    iget-boolean v5, v13, Lokhttp3/internal/ws/RealWebSocket;->failed:Z
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_a

    const/4 v6, 0x0

    if-eqz v5, :cond_0

    .line 429
    nop

    .end local v0    # "$i$a$-synchronized-RealWebSocket$writeOneFrame$1":I
    monitor-exit p0

    return v6

    .line 432
    .restart local v0    # "$i$a$-synchronized-RealWebSocket$writeOneFrame$1":I
    :cond_0
    :try_start_1
    iget-object v5, v13, Lokhttp3/internal/ws/RealWebSocket;->writer:Lokhttp3/internal/ws/WebSocketWriter;
    :try_end_1
    .catchall {:try_start_1 .. :try_end_1} :catchall_a

    move-object v10, v5

    .line 433
    .end local v1    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .local v10, "writer":Lokhttp3/internal/ws/WebSocketWriter;
    :try_start_2
    iget-object v1, v13, Lokhttp3/internal/ws/RealWebSocket;->pongQueue:Ljava/util/ArrayDeque;

    invoke-virtual {v1}, Ljava/util/ArrayDeque;->poll()Ljava/lang/Object;

    move-result-object v1

    check-cast v1, Lokio/ByteString;
    :try_end_2
    .catchall {:try_start_2 .. :try_end_2} :catchall_9

    move-object v9, v1

    .line 434
    .end local v2    # "pong":Lokio/ByteString;
    .local v9, "pong":Lokio/ByteString;
    if-nez v9, :cond_4

    .line 435
    :try_start_3
    iget-object v1, v13, Lokhttp3/internal/ws/RealWebSocket;->messageAndCloseQueue:Ljava/util/ArrayDeque;

    invoke-virtual {v1}, Ljava/util/ArrayDeque;->poll()Ljava/lang/Object;

    move-result-object v1

    iput-object v1, v14, Lkotlin/jvm/internal/Ref$ObjectRef;->element:Ljava/lang/Object;

    .line 436
    iget-object v1, v14, Lkotlin/jvm/internal/Ref$ObjectRef;->element:Ljava/lang/Object;

    instance-of v1, v1, Lokhttp3/internal/ws/RealWebSocket$Close;

    if-eqz v1, :cond_3

    .line 437
    iget v1, v13, Lokhttp3/internal/ws/RealWebSocket;->receivedCloseCode:I

    iput v1, v15, Lkotlin/jvm/internal/Ref$IntRef;->element:I

    .line 438
    iget-object v1, v13, Lokhttp3/internal/ws/RealWebSocket;->receivedCloseReason:Ljava/lang/String;

    iput-object v1, v12, Lkotlin/jvm/internal/Ref$ObjectRef;->element:Ljava/lang/Object;

    .line 439
    iget v1, v15, Lkotlin/jvm/internal/Ref$IntRef;->element:I
    :try_end_3
    .catchall {:try_start_3 .. :try_end_3} :catchall_2

    if-eq v1, v4, :cond_1

    .line 440
    :try_start_4
    iget-object v1, v13, Lokhttp3/internal/ws/RealWebSocket;->streams:Lokhttp3/internal/ws/RealWebSocket$Streams;

    iput-object v1, v11, Lkotlin/jvm/internal/Ref$ObjectRef;->element:Ljava/lang/Object;

    .line 441
    check-cast v3, Lokhttp3/internal/ws/RealWebSocket$Streams;

    iput-object v3, v13, Lokhttp3/internal/ws/RealWebSocket;->streams:Lokhttp3/internal/ws/RealWebSocket$Streams;

    .line 442
    iget-object v1, v13, Lokhttp3/internal/ws/RealWebSocket;->taskQueue:Lokhttp3/internal/concurrent/TaskQueue;

    invoke-virtual {v1}, Lokhttp3/internal/concurrent/TaskQueue;->shutdown()V
    :try_end_4
    .catchall {:try_start_4 .. :try_end_4} :catchall_0

    move/from16 v19, v0

    move-object/from16 v24, v9

    move-object/from16 v25, v10

    move-object/from16 v26, v11

    move-object/from16 v27, v12

    goto/16 :goto_0

    .line 427
    .end local v0    # "$i$a$-synchronized-RealWebSocket$writeOneFrame$1":I
    :catchall_0
    move-exception v0

    move-object v2, v9

    move-object v1, v10

    move-object v4, v11

    move-object v7, v12

    goto/16 :goto_3

    .line 445
    .restart local v0    # "$i$a$-synchronized-RealWebSocket$writeOneFrame$1":I
    :cond_1
    :try_start_5
    iget-object v1, v14, Lkotlin/jvm/internal/Ref$ObjectRef;->element:Ljava/lang/Object;

    if-eqz v1, :cond_2

    check-cast v1, Lokhttp3/internal/ws/RealWebSocket$Close;

    invoke-virtual {v1}, Lokhttp3/internal/ws/RealWebSocket$Close;->getCancelAfterCloseMillis()J

    move-result-wide v1

    move-wide v7, v1

    .line 446
    .local v7, "cancelAfterCloseMillis":J
    iget-object v1, v13, Lokhttp3/internal/ws/RealWebSocket;->taskQueue:Lokhttp3/internal/concurrent/TaskQueue;

    new-instance v2, Ljava/lang/StringBuilder;

    invoke-direct {v2}, Ljava/lang/StringBuilder;-><init>()V

    iget-object v3, v13, Lokhttp3/internal/ws/RealWebSocket;->name:Ljava/lang/String;

    invoke-virtual {v2, v3}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v3, " cancel"

    invoke-virtual {v2, v3}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v2}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v2

    sget-object v3, Ljava/util/concurrent/TimeUnit;->MILLISECONDS:Ljava/util/concurrent/TimeUnit;

    invoke-virtual {v3, v7, v8}, Ljava/util/concurrent/TimeUnit;->toNanos(J)J

    move-result-wide v3

    move-wide v5, v3

    .local v2, "name$iv":Ljava/lang/String;
    .local v5, "delayNanos$iv":J
    move-object v4, v1

    .line 580
    .local v4, "this_$iv":Lokhttp3/internal/concurrent/TaskQueue;
    const/16 v16, 0x1

    .local v16, "cancelable$iv":Z
    const/16 v17, 0x0

    .line 583
    .local v17, "$i$f$execute":I
    new-instance v18, Lokhttp3/internal/ws/RealWebSocket$writeOneFrame$$inlined$synchronized$lambda$1;
    :try_end_5
    .catchall {:try_start_5 .. :try_end_5} :catchall_2

    move-object/from16 v1, v18

    move/from16 v3, v16

    move/from16 v19, v0

    move-object v0, v4

    .end local v4    # "this_$iv":Lokhttp3/internal/concurrent/TaskQueue;
    .local v0, "this_$iv":Lokhttp3/internal/concurrent/TaskQueue;
    .local v19, "$i$a$-synchronized-RealWebSocket$writeOneFrame$1":I
    move-object v4, v2

    move-wide/from16 v20, v5

    .end local v5    # "delayNanos$iv":J
    .local v20, "delayNanos$iv":J
    move/from16 v5, v16

    move-object/from16 v6, p0

    move-wide/from16 v22, v7

    .end local v7    # "cancelAfterCloseMillis":J
    .local v22, "cancelAfterCloseMillis":J
    move-object v7, v10

    move-object v8, v9

    move-object/from16 v24, v9

    .end local v9    # "pong":Lokio/ByteString;
    .local v24, "pong":Lokio/ByteString;
    move-object v9, v14

    move-object/from16 v25, v10

    .end local v10    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .local v25, "writer":Lokhttp3/internal/ws/WebSocketWriter;
    move-object v10, v15

    move-object/from16 v26, v11

    .end local v11    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .local v26, "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    move-object v11, v12

    move-object/from16 v27, v12

    .end local v12    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .local v27, "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    move-object/from16 v12, v26

    :try_start_6
    invoke-direct/range {v1 .. v12}, Lokhttp3/internal/ws/RealWebSocket$writeOneFrame$$inlined$synchronized$lambda$1;-><init>(Ljava/lang/String;ZLjava/lang/String;ZLokhttp3/internal/ws/RealWebSocket;Lokhttp3/internal/ws/WebSocketWriter;Lokio/ByteString;Lkotlin/jvm/internal/Ref$ObjectRef;Lkotlin/jvm/internal/Ref$IntRef;Lkotlin/jvm/internal/Ref$ObjectRef;Lkotlin/jvm/internal/Ref$ObjectRef;)V

    move-object/from16 v1, v18

    check-cast v1, Lokhttp3/internal/concurrent/Task;

    .line 588
    nop

    .line 583
    move-wide/from16 v3, v20

    .end local v20    # "delayNanos$iv":J
    .local v3, "delayNanos$iv":J
    invoke-virtual {v0, v1, v3, v4}, Lokhttp3/internal/concurrent/TaskQueue;->schedule(Lokhttp3/internal/concurrent/Task;J)V

    .line 589
    nop

    .line 449
    .end local v0    # "this_$iv":Lokhttp3/internal/concurrent/TaskQueue;
    .end local v2    # "name$iv":Ljava/lang/String;
    .end local v3    # "delayNanos$iv":J
    .end local v16    # "cancelable$iv":Z
    .end local v17    # "$i$f$execute":I
    .end local v22    # "cancelAfterCloseMillis":J
    goto :goto_0

    .line 445
    .end local v19    # "$i$a$-synchronized-RealWebSocket$writeOneFrame$1":I
    .end local v24    # "pong":Lokio/ByteString;
    .end local v25    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .end local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .local v0, "$i$a$-synchronized-RealWebSocket$writeOneFrame$1":I
    .restart local v9    # "pong":Lokio/ByteString;
    .restart local v10    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .restart local v11    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v12    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    :cond_2
    move/from16 v19, v0

    move-object/from16 v24, v9

    move-object/from16 v25, v10

    move-object/from16 v26, v11

    move-object/from16 v27, v12

    .end local v0    # "$i$a$-synchronized-RealWebSocket$writeOneFrame$1":I
    .end local v9    # "pong":Lokio/ByteString;
    .end local v10    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .end local v11    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v12    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v19    # "$i$a$-synchronized-RealWebSocket$writeOneFrame$1":I
    .restart local v24    # "pong":Lokio/ByteString;
    .restart local v25    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .restart local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    new-instance v0, Lkotlin/TypeCastException;

    const-string v1, "null cannot be cast to non-null type okhttp3.internal.ws.RealWebSocket.Close"

    invoke-direct {v0, v1}, Lkotlin/TypeCastException;-><init>(Ljava/lang/String;)V

    .end local v14    # "messageOrClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v15    # "receivedCloseCode":Lkotlin/jvm/internal/Ref$IntRef;
    .end local v24    # "pong":Lokio/ByteString;
    .end local v25    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .end local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    throw v0

    .line 450
    .end local v19    # "$i$a$-synchronized-RealWebSocket$writeOneFrame$1":I
    .restart local v0    # "$i$a$-synchronized-RealWebSocket$writeOneFrame$1":I
    .restart local v9    # "pong":Lokio/ByteString;
    .restart local v10    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .restart local v11    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v12    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v14    # "messageOrClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v15    # "receivedCloseCode":Lkotlin/jvm/internal/Ref$IntRef;
    :cond_3
    move/from16 v19, v0

    move-object/from16 v24, v9

    move-object/from16 v25, v10

    move-object/from16 v26, v11

    move-object/from16 v27, v12

    .end local v0    # "$i$a$-synchronized-RealWebSocket$writeOneFrame$1":I
    .end local v9    # "pong":Lokio/ByteString;
    .end local v10    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .end local v11    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v12    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v19    # "$i$a$-synchronized-RealWebSocket$writeOneFrame$1":I
    .restart local v24    # "pong":Lokio/ByteString;
    .restart local v25    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .restart local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    iget-object v0, v14, Lkotlin/jvm/internal/Ref$ObjectRef;->element:Ljava/lang/Object;
    :try_end_6
    .catchall {:try_start_6 .. :try_end_6} :catchall_1

    if-nez v0, :cond_5

    .line 451
    nop

    .end local v19    # "$i$a$-synchronized-RealWebSocket$writeOneFrame$1":I
    monitor-exit p0

    return v6

    .line 427
    :catchall_1
    move-exception v0

    move-object/from16 v2, v24

    move-object/from16 v1, v25

    move-object/from16 v4, v26

    move-object/from16 v7, v27

    goto/16 :goto_3

    .end local v24    # "pong":Lokio/ByteString;
    .end local v25    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .end local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v9    # "pong":Lokio/ByteString;
    .restart local v10    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .restart local v11    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v12    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    :catchall_2
    move-exception v0

    move-object/from16 v24, v9

    move-object/from16 v25, v10

    move-object v4, v11

    move-object v7, v12

    move-object/from16 v2, v24

    move-object/from16 v1, v25

    .end local v9    # "pong":Lokio/ByteString;
    .end local v10    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .end local v11    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v12    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v24    # "pong":Lokio/ByteString;
    .restart local v25    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .restart local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    goto/16 :goto_3

    .line 434
    .end local v24    # "pong":Lokio/ByteString;
    .end local v25    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .end local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v0    # "$i$a$-synchronized-RealWebSocket$writeOneFrame$1":I
    .restart local v9    # "pong":Lokio/ByteString;
    .restart local v10    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .restart local v11    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v12    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    :cond_4
    move/from16 v19, v0

    move-object/from16 v24, v9

    move-object/from16 v25, v10

    move-object/from16 v26, v11

    move-object/from16 v27, v12

    .line 452
    .end local v0    # "$i$a$-synchronized-RealWebSocket$writeOneFrame$1":I
    .end local v9    # "pong":Lokio/ByteString;
    .end local v10    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .end local v11    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v12    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v19    # "$i$a$-synchronized-RealWebSocket$writeOneFrame$1":I
    .restart local v24    # "pong":Lokio/ByteString;
    .restart local v25    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .restart local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    :cond_5
    :goto_0
    nop

    .line 454
    nop

    .end local v19    # "$i$a$-synchronized-RealWebSocket$writeOneFrame$1":I
    :try_start_7
    sget-object v0, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;
    :try_end_7
    .catchall {:try_start_7 .. :try_end_7} :catchall_8

    .line 427
    monitor-exit p0

    .line 456
    nop

    .line 457
    move-object/from16 v1, v24

    .end local v24    # "pong":Lokio/ByteString;
    .local v1, "pong":Lokio/ByteString;
    if-eqz v1, :cond_7

    .line 458
    move-object/from16 v3, v25

    .end local v25    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .local v3, "writer":Lokhttp3/internal/ws/WebSocketWriter;
    if-nez v3, :cond_6

    :try_start_8
    invoke-static {}, Lkotlin/jvm/internal/Intrinsics;->throwNpe()V

    :cond_6
    invoke-virtual {v3, v1}, Lokhttp3/internal/ws/WebSocketWriter;->writePong(Lokio/ByteString;)V
    :try_end_8
    .catchall {:try_start_8 .. :try_end_8} :catchall_3

    move-object/from16 v4, v26

    move-object/from16 v7, v27

    goto/16 :goto_1

    .line 483
    :catchall_3
    move-exception v0

    move-object/from16 v4, v26

    move-object/from16 v7, v27

    goto/16 :goto_2

    .line 459
    .end local v3    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .restart local v25    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    :cond_7
    move-object/from16 v3, v25

    .end local v25    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .restart local v3    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    :try_start_9
    iget-object v0, v14, Lkotlin/jvm/internal/Ref$ObjectRef;->element:Ljava/lang/Object;

    instance-of v0, v0, Lokhttp3/internal/ws/RealWebSocket$Message;
    :try_end_9
    .catchall {:try_start_9 .. :try_end_9} :catchall_7

    if-eqz v0, :cond_b

    .line 460
    :try_start_a
    iget-object v0, v14, Lkotlin/jvm/internal/Ref$ObjectRef;->element:Ljava/lang/Object;

    if-eqz v0, :cond_a

    check-cast v0, Lokhttp3/internal/ws/RealWebSocket$Message;

    invoke-virtual {v0}, Lokhttp3/internal/ws/RealWebSocket$Message;->getData()Lokio/ByteString;

    move-result-object v0

    move-object v2, v0

    .line 461
    .local v2, "data":Lokio/ByteString;
    nop

    .line 462
    nop

    .line 461
    if-nez v3, :cond_8

    invoke-static {}, Lkotlin/jvm/internal/Intrinsics;->throwNpe()V

    .line 462
    :cond_8
    iget-object v0, v14, Lkotlin/jvm/internal/Ref$ObjectRef;->element:Ljava/lang/Object;

    if-eqz v0, :cond_9

    check-cast v0, Lokhttp3/internal/ws/RealWebSocket$Message;

    invoke-virtual {v0}, Lokhttp3/internal/ws/RealWebSocket$Message;->getFormatOpcode()I

    move-result v0

    invoke-virtual {v2}, Lokio/ByteString;->size()I

    move-result v4

    int-to-long v4, v4

    .line 461
    invoke-virtual {v3, v0, v4, v5}, Lokhttp3/internal/ws/WebSocketWriter;->newMessageSink(IJ)Lokio/Sink;

    move-result-object v0

    .line 462
    invoke-static {v0}, Lokio/Okio;->buffer(Lokio/Sink;)Lokio/BufferedSink;

    move-result-object v0

    .line 461
    move-object v4, v0

    .line 463
    .local v4, "sink":Lokio/BufferedSink;
    invoke-interface {v4, v2}, Lokio/BufferedSink;->write(Lokio/ByteString;)Lokio/BufferedSink;

    .line 464
    invoke-interface {v4}, Lokio/BufferedSink;->close()V

    .line 465
    monitor-enter p0
    :try_end_a
    .catchall {:try_start_a .. :try_end_a} :catchall_3

    const/4 v0, 0x0

    .line 466
    .local v0, "$i$a$-synchronized-RealWebSocket$writeOneFrame$2":I
    :try_start_b
    iget-wide v5, v13, Lokhttp3/internal/ws/RealWebSocket;->queueSize:J

    invoke-virtual {v2}, Lokio/ByteString;->size()I

    move-result v7

    int-to-long v7, v7

    sub-long/2addr v5, v7

    iput-wide v5, v13, Lokhttp3/internal/ws/RealWebSocket;->queueSize:J

    .line 467
    .end local v0    # "$i$a$-synchronized-RealWebSocket$writeOneFrame$2":I
    sget-object v0, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;
    :try_end_b
    .catchall {:try_start_b .. :try_end_b} :catchall_4

    .line 465
    :try_start_c
    monitor-exit p0

    .end local v2    # "data":Lokio/ByteString;
    .end local v4    # "sink":Lokio/BufferedSink;
    move-object/from16 v4, v26

    move-object/from16 v7, v27

    goto :goto_1

    .restart local v2    # "data":Lokio/ByteString;
    .restart local v4    # "sink":Lokio/BufferedSink;
    :catchall_4
    move-exception v0

    monitor-exit p0

    .end local v1    # "pong":Lokio/ByteString;
    .end local v3    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .end local v14    # "messageOrClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v15    # "receivedCloseCode":Lkotlin/jvm/internal/Ref$IntRef;
    .end local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    throw v0

    .line 462
    .end local v4    # "sink":Lokio/BufferedSink;
    .restart local v1    # "pong":Lokio/ByteString;
    .restart local v3    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .restart local v14    # "messageOrClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v15    # "receivedCloseCode":Lkotlin/jvm/internal/Ref$IntRef;
    .restart local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    :cond_9
    new-instance v0, Lkotlin/TypeCastException;

    const-string v4, "null cannot be cast to non-null type okhttp3.internal.ws.RealWebSocket.Message"

    invoke-direct {v0, v4}, Lkotlin/TypeCastException;-><init>(Ljava/lang/String;)V

    .end local v1    # "pong":Lokio/ByteString;
    .end local v3    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .end local v14    # "messageOrClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v15    # "receivedCloseCode":Lkotlin/jvm/internal/Ref$IntRef;
    .end local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    throw v0

    .line 460
    .end local v2    # "data":Lokio/ByteString;
    .restart local v1    # "pong":Lokio/ByteString;
    .restart local v3    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .restart local v14    # "messageOrClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v15    # "receivedCloseCode":Lkotlin/jvm/internal/Ref$IntRef;
    .restart local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    :cond_a
    new-instance v0, Lkotlin/TypeCastException;

    const-string v2, "null cannot be cast to non-null type okhttp3.internal.ws.RealWebSocket.Message"

    invoke-direct {v0, v2}, Lkotlin/TypeCastException;-><init>(Ljava/lang/String;)V

    .end local v1    # "pong":Lokio/ByteString;
    .end local v3    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .end local v14    # "messageOrClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v15    # "receivedCloseCode":Lkotlin/jvm/internal/Ref$IntRef;
    .end local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    throw v0
    :try_end_c
    .catchall {:try_start_c .. :try_end_c} :catchall_3

    .line 468
    .restart local v1    # "pong":Lokio/ByteString;
    .restart local v3    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .restart local v14    # "messageOrClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v15    # "receivedCloseCode":Lkotlin/jvm/internal/Ref$IntRef;
    .restart local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    :cond_b
    :try_start_d
    iget-object v0, v14, Lkotlin/jvm/internal/Ref$ObjectRef;->element:Ljava/lang/Object;

    instance-of v0, v0, Lokhttp3/internal/ws/RealWebSocket$Close;

    if-eqz v0, :cond_11

    .line 469
    iget-object v0, v14, Lkotlin/jvm/internal/Ref$ObjectRef;->element:Ljava/lang/Object;

    if-eqz v0, :cond_10

    check-cast v0, Lokhttp3/internal/ws/RealWebSocket$Close;
    :try_end_d
    .catchall {:try_start_d .. :try_end_d} :catchall_7

    .line 470
    .local v0, "close":Lokhttp3/internal/ws/RealWebSocket$Close;
    if-nez v3, :cond_c

    :try_start_e
    invoke-static {}, Lkotlin/jvm/internal/Intrinsics;->throwNpe()V
    :try_end_e
    .catchall {:try_start_e .. :try_end_e} :catchall_3

    :cond_c
    :try_start_f
    invoke-virtual {v0}, Lokhttp3/internal/ws/RealWebSocket$Close;->getCode()I

    move-result v2

    invoke-virtual {v0}, Lokhttp3/internal/ws/RealWebSocket$Close;->getReason()Lokio/ByteString;

    move-result-object v4

    invoke-virtual {v3, v2, v4}, Lokhttp3/internal/ws/WebSocketWriter;->writeClose(ILokio/ByteString;)V
    :try_end_f
    .catchall {:try_start_f .. :try_end_f} :catchall_7

    .line 473
    move-object/from16 v4, v26

    .end local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .local v4, "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    :try_start_10
    iget-object v2, v4, Lkotlin/jvm/internal/Ref$ObjectRef;->element:Ljava/lang/Object;

    check-cast v2, Lokhttp3/internal/ws/RealWebSocket$Streams;

    if-eqz v2, :cond_e

    .line 474
    iget-object v2, v13, Lokhttp3/internal/ws/RealWebSocket;->listener:Lokhttp3/WebSocketListener;

    move-object v5, v13

    check-cast v5, Lokhttp3/WebSocket;

    iget v6, v15, Lkotlin/jvm/internal/Ref$IntRef;->element:I
    :try_end_10
    .catchall {:try_start_10 .. :try_end_10} :catchall_5

    move-object/from16 v7, v27

    .end local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .local v7, "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    :try_start_11
    iget-object v8, v7, Lkotlin/jvm/internal/Ref$ObjectRef;->element:Ljava/lang/Object;

    check-cast v8, Ljava/lang/String;

    if-nez v8, :cond_d

    invoke-static {}, Lkotlin/jvm/internal/Intrinsics;->throwNpe()V

    :cond_d
    invoke-virtual {v2, v5, v6, v8}, Lokhttp3/WebSocketListener;->onClosed(Lokhttp3/WebSocket;ILjava/lang/String;)V
    :try_end_11
    .catchall {:try_start_11 .. :try_end_11} :catchall_6

    .end local v0    # "close":Lokhttp3/internal/ws/RealWebSocket$Close;
    goto :goto_1

    .line 473
    .end local v7    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v0    # "close":Lokhttp3/internal/ws/RealWebSocket$Close;
    .restart local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    :cond_e
    move-object/from16 v7, v27

    .line 478
    .end local v0    # "close":Lokhttp3/internal/ws/RealWebSocket$Close;
    .end local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v7    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    :goto_1
    nop

    .line 480
    const/4 v0, 0x1

    .line 482
    iget-object v2, v4, Lkotlin/jvm/internal/Ref$ObjectRef;->element:Ljava/lang/Object;

    check-cast v2, Lokhttp3/internal/ws/RealWebSocket$Streams;

    if-eqz v2, :cond_f

    check-cast v2, Ljava/io/Closeable;

    invoke-static {v2}, Lokhttp3/internal/Util;->closeQuietly(Ljava/io/Closeable;)V

    :cond_f
    return v0

    .line 483
    .end local v7    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    :catchall_5
    move-exception v0

    move-object/from16 v7, v27

    .end local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v7    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    goto :goto_2

    .line 469
    .end local v4    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v7    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    :cond_10
    move-object/from16 v4, v26

    move-object/from16 v7, v27

    .end local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v4    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v7    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    :try_start_12
    new-instance v0, Lkotlin/TypeCastException;

    const-string v2, "null cannot be cast to non-null type okhttp3.internal.ws.RealWebSocket.Close"

    invoke-direct {v0, v2}, Lkotlin/TypeCastException;-><init>(Ljava/lang/String;)V

    .end local v1    # "pong":Lokio/ByteString;
    .end local v3    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .end local v4    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v7    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v14    # "messageOrClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v15    # "receivedCloseCode":Lkotlin/jvm/internal/Ref$IntRef;
    throw v0

    .line 477
    .restart local v1    # "pong":Lokio/ByteString;
    .restart local v3    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .restart local v14    # "messageOrClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v15    # "receivedCloseCode":Lkotlin/jvm/internal/Ref$IntRef;
    .restart local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    :cond_11
    move-object/from16 v4, v26

    move-object/from16 v7, v27

    .end local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v4    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v7    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    new-instance v0, Ljava/lang/AssertionError;

    invoke-direct {v0}, Ljava/lang/AssertionError;-><init>()V

    check-cast v0, Ljava/lang/Throwable;

    .end local v1    # "pong":Lokio/ByteString;
    .end local v3    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .end local v4    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v7    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v14    # "messageOrClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v15    # "receivedCloseCode":Lkotlin/jvm/internal/Ref$IntRef;
    throw v0
    :try_end_12
    .catchall {:try_start_12 .. :try_end_12} :catchall_6

    .line 483
    .restart local v1    # "pong":Lokio/ByteString;
    .restart local v3    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .restart local v4    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v7    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v14    # "messageOrClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v15    # "receivedCloseCode":Lkotlin/jvm/internal/Ref$IntRef;
    :catchall_6
    move-exception v0

    goto :goto_2

    .end local v4    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v7    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    :catchall_7
    move-exception v0

    move-object/from16 v4, v26

    move-object/from16 v7, v27

    .line 482
    .end local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v4    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v7    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    :goto_2
    iget-object v2, v4, Lkotlin/jvm/internal/Ref$ObjectRef;->element:Ljava/lang/Object;

    check-cast v2, Lokhttp3/internal/ws/RealWebSocket$Streams;

    if-eqz v2, :cond_12

    check-cast v2, Ljava/io/Closeable;

    invoke-static {v2}, Lokhttp3/internal/Util;->closeQuietly(Ljava/io/Closeable;)V

    :cond_12
    throw v0

    .line 427
    .end local v1    # "pong":Lokio/ByteString;
    .end local v3    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .end local v4    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v7    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v24    # "pong":Lokio/ByteString;
    .restart local v25    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .restart local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    :catchall_8
    move-exception v0

    move-object/from16 v1, v24

    move-object/from16 v3, v25

    move-object/from16 v4, v26

    move-object/from16 v7, v27

    move-object v2, v1

    move-object v1, v3

    .end local v24    # "pong":Lokio/ByteString;
    .end local v25    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .end local v26    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v27    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v1    # "pong":Lokio/ByteString;
    .restart local v3    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .restart local v4    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v7    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    goto :goto_3

    .end local v1    # "pong":Lokio/ByteString;
    .end local v3    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .end local v4    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v7    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .local v2, "pong":Lokio/ByteString;
    .restart local v10    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .restart local v11    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v12    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    :catchall_9
    move-exception v0

    move-object v3, v10

    move-object v4, v11

    move-object v7, v12

    move-object v1, v3

    .end local v10    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .end local v11    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v12    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v3    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .restart local v4    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v7    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    goto :goto_3

    .end local v3    # "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .end local v4    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v7    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .local v1, "writer":Lokhttp3/internal/ws/WebSocketWriter;
    .restart local v11    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v12    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    :catchall_a
    move-exception v0

    move-object v4, v11

    move-object v7, v12

    .end local v11    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .end local v12    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v4    # "streamsToClose":Lkotlin/jvm/internal/Ref$ObjectRef;
    .restart local v7    # "receivedCloseReason":Lkotlin/jvm/internal/Ref$ObjectRef;
    :goto_3
    monitor-exit p0

    throw v0
.end method

.method public final writePingFrame$okhttp()V
    .locals 7

    .line 487
    const/4 v0, 0x0

    .line 488
    .local v0, "writer":Lokhttp3/internal/ws/WebSocketWriter;
    const/4 v1, 0x0

    .line 489
    .local v1, "failedPing":I
    monitor-enter p0

    const/4 v2, 0x0

    .line 490
    .local v2, "$i$a$-synchronized-RealWebSocket$writePingFrame$1":I
    :try_start_0
    iget-boolean v3, p0, Lokhttp3/internal/ws/RealWebSocket;->failed:Z
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    if-eqz v3, :cond_0

    .end local v2    # "$i$a$-synchronized-RealWebSocket$writePingFrame$1":I
    monitor-exit p0

    return-void

    .line 491
    .restart local v2    # "$i$a$-synchronized-RealWebSocket$writePingFrame$1":I
    :cond_0
    :try_start_1
    iget-object v3, p0, Lokhttp3/internal/ws/RealWebSocket;->writer:Lokhttp3/internal/ws/WebSocketWriter;

    move-object v0, v3

    .line 492
    iget-boolean v3, p0, Lokhttp3/internal/ws/RealWebSocket;->awaitingPong:Z

    const/4 v4, -0x1

    if-eqz v3, :cond_1

    iget v3, p0, Lokhttp3/internal/ws/RealWebSocket;->sentPingCount:I

    goto :goto_0

    :cond_1
    const/4 v3, -0x1

    :goto_0
    move v1, v3

    .line 493
    iget v3, p0, Lokhttp3/internal/ws/RealWebSocket;->sentPingCount:I

    const/4 v5, 0x1

    add-int/2addr v3, v5

    iput v3, p0, Lokhttp3/internal/ws/RealWebSocket;->sentPingCount:I

    .line 494
    iput-boolean v5, p0, Lokhttp3/internal/ws/RealWebSocket;->awaitingPong:Z

    .line 495
    .end local v2    # "$i$a$-synchronized-RealWebSocket$writePingFrame$1":I
    sget-object v2, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;
    :try_end_1
    .catchall {:try_start_1 .. :try_end_1} :catchall_0

    .line 489
    monitor-exit p0

    .line 497
    const/4 v2, 0x0

    if-eq v1, v4, :cond_2

    .line 498
    new-instance v3, Ljava/net/SocketTimeoutException;

    new-instance v4, Ljava/lang/StringBuilder;

    invoke-direct {v4}, Ljava/lang/StringBuilder;-><init>()V

    const-string v5, "sent ping but didn\'t receive pong within "

    invoke-virtual {v4, v5}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    .line 499
    iget-wide v5, p0, Lokhttp3/internal/ws/RealWebSocket;->pingIntervalMillis:J

    invoke-virtual {v4, v5, v6}, Ljava/lang/StringBuilder;->append(J)Ljava/lang/StringBuilder;

    const-string v5, "ms (after "

    invoke-virtual {v4, v5}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    add-int/lit8 v5, v1, -0x1

    invoke-virtual {v4, v5}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    const-string v5, " successful ping/pongs)"

    invoke-virtual {v4, v5}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v4}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v4

    .line 498
    invoke-direct {v3, v4}, Ljava/net/SocketTimeoutException;-><init>(Ljava/lang/String;)V

    check-cast v3, Ljava/lang/Exception;

    .line 499
    nop

    .line 498
    invoke-virtual {p0, v3, v2}, Lokhttp3/internal/ws/RealWebSocket;->failWebSocket(Ljava/lang/Exception;Lokhttp3/Response;)V

    .line 500
    return-void

    .line 503
    :cond_2
    nop

    .line 504
    if-nez v0, :cond_3

    :try_start_2
    invoke-static {}, Lkotlin/jvm/internal/Intrinsics;->throwNpe()V

    :cond_3
    sget-object v3, Lokio/ByteString;->EMPTY:Lokio/ByteString;

    invoke-virtual {v0, v3}, Lokhttp3/internal/ws/WebSocketWriter;->writePing(Lokio/ByteString;)V
    :try_end_2
    .catch Ljava/io/IOException; {:try_start_2 .. :try_end_2} :catch_0

    goto :goto_1

    .line 505
    :catch_0
    move-exception v3

    .line 506
    .local v3, "e":Ljava/io/IOException;
    move-object v4, v3

    check-cast v4, Ljava/lang/Exception;

    invoke-virtual {p0, v4, v2}, Lokhttp3/internal/ws/RealWebSocket;->failWebSocket(Ljava/lang/Exception;Lokhttp3/Response;)V

    .line 507
    .end local v3    # "e":Ljava/io/IOException;
    :goto_1
    nop

    .line 508
    return-void

    .line 489
    :catchall_0
    move-exception v2

    monitor-exit p0

    throw v2
.end method
