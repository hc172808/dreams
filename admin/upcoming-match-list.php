  ...
  if($row_cnt==1)
  {
  	...
  	if($mtResult['parti1']=='' & $mtResult['parti2']=='')
  	{
		...
		if ($conn->query($sql) === TRUE) {
	        ...
	        header("Location:upcoming-match-list");
	        exit();
		} else {
...
<html lang="en">
	...
	<head><base href="">
		<meta charset="utf-8" />
		<title>Upcoming Match List | <?php echo $appDetRes['app_name']; ?></title>
						...
						<div class="subheader py-2 py-lg-4 subheader-solid" id="kt_subheader">
							<div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
								...
								<div class="d-flex align-items-center flex-wrap mr-2">
									<!--begin::Page Title-->
									<h5 class="text-dark font-weight-bold mt-2 mb-2 mr-5">Upcoming  Match List</h5>
									<!--end::Page Title-->
								</div>
								<!--end::Info-->
								<!--begin::Toolbar-->
					...
					<script type="text/javascript">
				    	$(document).ready(function() {
						    $('#example').DataTable( {
						        "processing": true,
						        "serverSide": true,
						        "ajax": "data-processing/getMatch.php?mtstatus=1",
						        "columnDefs": [{"render": function ( data, type, full, meta ) {
					                    var buttonID = full[0];
					                    var match_status_temp = full[6];
					                    if(match_status_temp === '1')
					                    {
					                     	return '<a href="create-match?uid='+buttonID+'" class="mr-5" ><i class="far fa-edit"></i></a> &nbsp;&nbsp;<a href="upcoming-match-list?did='+buttonID+'" class="mr-5" onclick="return checkDelete()"><i class="fas fa-times-circle"></i></a>';
					                 	}
					                 	else
					                 	{
